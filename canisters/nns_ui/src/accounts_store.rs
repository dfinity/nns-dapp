...
mod tests {
    ...
    fn prune_transactions() {
        ...
        let unknown_account =
            AccountIdentifier::from(PrincipalId::from_str(TEST_ACCOUNT_4).unwrap());
        ...
        for account in store.accounts.iter().map(|a| a.as_ref().unwrap()) {
            transaction_indexes_remaining
                .append(account.default_account_transactions.clone().as_mut());
