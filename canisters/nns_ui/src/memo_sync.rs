...
pub async fn init() {
    if let Ok(archive_index) = canisters::ledger::get_archive_index().await {
        let block_heights_with_missing_memo = STATE
            .read()
            .unwrap()
            .accounts_store
            .get_block_heights_with_missing_memo();

        DATA.with(|d| {
            *d.borrow_mut() = Some(MemoSyncData::new(
                block_heights_with_missing_memo,
                archive_index,
            ))
        });
    }
...
pub async fn sync_next() -> Result<(), String> {
    if let Some((block_height, canister_id)) = take_next() {
        let memo = canisters::ledger::get_block(canister_id, block_height)
            .await?
            .decode()
            .unwrap()
            .transaction
