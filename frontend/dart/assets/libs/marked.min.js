...
!(function (e, t) {
  ...
  function _(e, t, u, n) {
    var r = t.href,
      ...
      t = e[1].replace(/\\([\[\]])/g, "$1");
    if ("!" === e[0].charAt(0))
      return { type: "image", raw: u, href: r, title: i, text: D(t) };
    n.state.inLink = !0;
    t = {
      type: "link",
      raw: u,
          ...
          if (t) {
            ...
            for (var d = f.items.length, C = 0; C < d; C++) {
              (this.lexer.state.top = !1),
                (f.items[C].tokens = this.lexer.blockTokens(
                  f.items[C].text,
                  []
                ));
        ...
        (t.link = function (e) {
          ...
          if (t) {
            ...
            if (!this.options.pedantic && /^</.test(e)) {
              ...
              -1 < n &&
                ((r = (0 === t[0].indexOf("!") ? 5 : 4) + t[1].length + n),
  ...
  (S._punctuation = "!\"#$%&'()+\\-.,/:;<=>?@\\[\\]`^{|}~"),
    (S.punctuation = p(S.punctuation)
      .replace(/punctuation/g, S._punctuation)
      .getRegex()),
    (S.blockSkip = /\[[^\]]*?\]\([^\)]*?\)|`[^`]*?`|<[^>]*?>/g),
    (S.escapedEmSt = /\\\*|\\_/g),
    (S._comment = p($._comment).replace("(?:--\x3e|$)", "--\x3e").getRegex()),
    (S.emStrong.lDelim = p(S.emStrong.lDelim)
      .replace(/punct/g, S._punctuation)
      .getRegex()),
    (S.emStrong.rDelimAst = p(S.emStrong.rDelimAst, "g")
      .replace(/punct/g, S._punctuation)
      .getRegex()),
    (S.emStrong.rDelimUnd = p(S.emStrong.rDelimUnd, "g")
      .replace(/punct/g, S._punctuation)
      .getRegex()),
    (S._escapes = /\\([!"#$%&'()*+,\-./:;<=>?@\[\]\\^_`{|}~])/g),
    ...
    (S.link = p(S.link)
      ...
      .getRegex()),
    (S.reflink = p(S.reflink)
      .replace("label", S._label)
      .replace("ref", $._label)
      .getRegex()),
    (S.nolink = p(S.nolink).replace("ref", $._label).getRegex()),
    (S.reflinkSearch = p(S.reflinkSearch, "g")
              ...
              switch (o.type) {
                ...
                case "heading":
                  d += this.renderer.heading(
                    ...
                    o.depth,
                    m(this.parseInline(o.tokens, this.textRenderer)),
                    this.slugger
                  );
                  continue;
                case "code":
  ...
  (j.options = j.setOptions =
        ...
        !(function () {
          ...
          switch ((l.call(j, t), t.type)) {
            ...
            default:
              ...
              j.defaults.extensions.childTokens[t.type]
                ? j.defaults.extensions.childTokens[t.type].forEach(function (
                    e
                  ) {
                    j.walkTokens(t[e], l);
                  })
    ...
    (j.parseInline = function (e, t) {
      ...
      try {
        ...
        return (
          t.walkTokens && j.walkTokens(u, t.walkTokens), L.parseInline(u, t)
        );
      } catch (e) {
        if (
          ((e.message +=
