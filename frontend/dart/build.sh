...
if [[ $DEPLOY_ENV = "mainnet" ]]; then
  ...
  flutter build web --web-renderer html --release --no-sound-null-safety --pwa-strategy=none --dart-define=DEPLOY_ENV=staging --dart-define=REDIRECT_TO_LEGACY="${REDIRECT_TO_LEGACY:-true}"
fi
