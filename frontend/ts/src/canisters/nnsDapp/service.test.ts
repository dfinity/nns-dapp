...
describe("getAccount", () => {
  test("respects the certified flag", () => {
    ...
    uncertified.get_account.mockReturnValue(
...
describe("getTransactions", () => {
  test("respects the certified flag", () => {
    ...
    uncertified.get_transactions.mockReturnValue(
      Promise.resolve(uncertifiedResponse)
    );
    ...
    certified.get_transactions.mockReturnValue(
      Promise.resolve(certifiedResponse)
    );

    const dapp = new Service(uncertified, certified);
    expect(
      dapp.getTransactions(mock<GetTransactionsRequest>(), false)
    ).resolves.toStrictEqual(uncertifiedResponse);
    expect(
      dapp.getTransactions(mock<GetTransactionsRequest>(), true)
    ).resolves.toStrictEqual(certifiedResponse);
