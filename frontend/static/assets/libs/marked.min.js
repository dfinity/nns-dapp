...
function escape(e, t) {
	if (t) {
		if (escapeTest.test(e)) return e.replace(escapeReplace, getEscapeReplacement);
...
function edit(e, t) {
	...
	const n = {
		replace: (t, r) => (
			(r = r.source || r), (r = r.replace(caret, '$1')), (e = e.replace(t, r)), n
		),
...
function cleanUrl(e, t, n) {
	if (e) {
		let e;
		try {
			e = decodeURIComponent(unescape(n)).replace(nonWordAndColonTest, '').toLowerCase();
		} catch (e) {
			return null;
		}
...
function resolveUrl(e, t) {
	baseUrls[' ' + e] ||
		(justDomain.test(e) ? (baseUrls[' ' + e] = e + '/') : (baseUrls[' ' + e] = rtrim(e, '/', !0))),
		(e = baseUrls[' ' + e]);
	const n = -1 === e.indexOf(':');
...
function outputLink(e, t, n, r) {
	...
	if ('!' !== e[0].charAt(0)) {
		r.state.inLink = !0;
		const e = { type: 'link', raw: n, href: s, title: i, text: l, tokens: r.inlineTokens(l, []) };
		return (r.state.inLink = !1), e;
	}
	return { type: 'image', raw: n, href: s, title: i, text: escape(l) };
}
...
function marked(e, t, n) {
	...
	if (
		...
		const i = function (e) {
			...
			if (!e)
				try {
					t.walkTokens && marked.walkTokens(s, t.walkTokens), (i = Parser.parse(s, t));
				} catch (t) {
					e = t;
				}
		...
		let l = 0;
		return (
			marked.walkTokens(s, function (e) {
				'code' === e.type &&
					(l++,
					setTimeout(() => {
	...
	try {
		const n = Lexer.lex(e, t);
		return t.walkTokens && marked.walkTokens(n, t.walkTokens), Parser.parse(n, t);
	} catch (e) {
...
class Tokenizer {
	...
	list(e) {
		...
		if (t) {
			...
			(k = d ? `\\d{1,9}\\${k.slice(-1)}` : `\\${k}`),
				this.options.pedantic && (k = d ? k : '[*+-]');
			const m = new RegExp(`^( {0,3}${k})((?:[\t ][^\\n]*)?(?:\\n|$))`);
			for (; e && ((g = !1), (t = m.exec(e))) && !this.rules.block.hr.test(e); ) {
				if (
					((n = t[0]),
				...
				f.loose || (o ? (f.loose = !0) : /\n *\n *$/.test(n) && (o = !0)),
					...
					f.items.push({ type: 'list_item', raw: n, task: !!r, checked: s, loose: !1, text: u }),
					(f.raw += n);
			}
			...
			for (l = 0; l < x; l++) {
				...
				!f.loose && e.length && t && ((f.loose = !0), (f.items[l].loose = !0));
			}
			return f;
		}
	}
		...
		if (t) {
			...
			return (
				this.options.sanitize &&
					((e.type = 'paragraph'),
					(e.text = this.options.sanitizer ? this.options.sanitizer(t[0]) : escape(t[0])),
					(e.tokens = []),
	...
	table(e) {
		...
		if (t) {
			const e = {
				type: 'table',
				header: splitCells(t[1]).map((e) => ({ text: e })),
			...
			if (e.header.length === e.align.length) {
				...
				for (l = e.rows.length, n = 0; n < l; n++)
					e.rows[n] = splitCells(e.rows[n], e.header.length).map((e) => ({ text: e }));
	...
	autolink(e, t) {
		...
		if (n) {
			...
			return (
				'@' === n[2]
					? ((e = escape(this.options.mangle ? t(n[1]) : n[1])), (r = 'mailto:' + e))
					: ((e = escape(n[1])), (r = e)),
	...
	url(e, t) {
		...
		if ((n = this.rules.inline.url.exec(e))) {
			let e, r;
			if ('@' === n[2]) (e = escape(this.options.mangle ? t(n[0]) : n[0])), (r = 'mailto:' + e);
			else {
				let t;
				do {
					(t = n[0]), (n[0] = this.rules.inline._backpedal.exec(n[0])[0]);
				} while (t !== n[0]);
				(e = escape(n[0])), (r = 'www.' === n[1] ? 'http://' + e : e);
			}
			return {
				type: 'link',
				raw: n[0],
				...
				(e = this.lexer.state.inRawBlock
					? this.options.sanitize
						? this.options.sanitizer
							...
							: escape(n[0])
						: n[0]
					: escape(this.options.smartypants ? t(n[0]) : n[0])),
...
class Lexer {
	constructor(e) {
		...
		this.options.pedantic
			...
			: this.options.gfm &&
			  ((t.block = block.gfm),
			  this.options.breaks ? (t.inline = inline.breaks) : (t.inline = inline.gfm)),
	...
	inlineTokens(e, t = []) {
		...
		if (this.tokens.links) {
			...
			if (e.length > 0)
				for (; null != (i = this.tokenizer.rules.inline.reflinkSearch.exec(o)); )
		...
		for (; null != (i = this.tokenizer.rules.inline.escapedEmSt.exec(o)); )
			o = o.slice(0, i.index) + '++' + o.slice(this.tokenizer.rules.inline.escapedEmSt.lastIndex);
		for (; e; )
			if (
				(l || (a = ''),
				(l = !1),
				...
				if ((n = this.tokenizer.escape(e))) (e = e.substring(n.raw.length)), t.push(n);
				...
				else if ((n = this.tokenizer.autolink(e, mangle)))
					(e = e.substring(n.raw.length)), t.push(n);
				else if (this.state.inLink || !(n = this.tokenizer.url(e, mangle))) {
					...
					if ((n = this.tokenizer.inlineText(s, smartypants)))
						(e = e.substring(n.raw.length)),
...
class Renderer {
	...
	heading(e, t, n, r) {
		if (this.options.headerIds) {
			const s = this.options.headerPrefix + r.slug(n);
	...
	link(e, t, n) {
		if (((e = cleanUrl(this.options.sanitize, this.options.baseUrl, e)), null === e)) return n;
		let r = '<a href="' + escape(e) + '"';
		...
		for (n = 0; n < y; n++)
			if (
				...
				this.options.extensions &&
					...
					((w = this.options.extensions.renderers[h.type].call({ parser: this }, h)),
					!1 !== w ||
						![
							'space',
							'hr',
				...
				switch (h.type) {
					...
					case 'code':
						...
						continue;
					case 'table':
						for (c = '', o = '', i = h.header.length, r = 0; r < i; r++)
							o += this.renderer.tablecell(this.parseInline(h.header[r].tokens), {
								header: !0,
								align: h.align[r]
							});
						for (c += this.renderer.tablerow(o), p = '', i = h.rows.length, r = 0; r < i; r++) {
							for (a = h.rows[r], o = '', l = a.length, s = 0; s < l; s++)
						...
						continue;
					case 'blockquote':
						(p = this.parse(h.tokens)), (_ += this.renderer.blockquote(p));
						continue;
					case 'list':
						for (
							u = h.ordered, g = h.start, k = h.loose, i = h.items.length, p = '', r = 0;
							r < i;
							r++
						)
							(f = h.items[r]),
								...
								f.task &&
									...
									k
										? f.tokens.length > 0 && 'paragraph' === f.tokens[0].type
											? ((f.tokens[0].text = b + ' ' + f.tokens[0].text),
											  f.tokens[0].tokens &&
													f.tokens[0].tokens.length > 0 &&
													'text' === f.tokens[0].tokens[0].type &&
													(f.tokens[0].tokens[0].text = b + ' ' + f.tokens[0].tokens[0].text))
											: f.tokens.unshift({ type: 'text', text: b })
										: (d += b)),
								(d += this.parse(f.tokens, k)),
								(p += this.renderer.listitem(d, x, m));
						_ += this.renderer.list(p, u, g);
						continue;
					case 'html':
					...
					case 'text':
						for (
							p = h.tokens ? this.parseInline(h.tokens) : h.text;
		...
		for (n = 0; n < l; n++)
			if (
				...
				this.options.extensions &&
					...
					((s = this.options.extensions.renderers[r.type].call({ parser: this }, r)),
					!1 !== s ||
						![
							'escape',
							'html',
...
(marked.options = marked.setOptions =
	...
	(marked.use = function (...e) {
		const t = merge({}, ...e),
			n = marked.defaults.extensions || { renderers: {}, childTokens: {} };
		let r;
		e.forEach((e) => {
			if (
				(e.extensions &&
			...
			r && (t.extensions = n), marked.setOptions(t);
		});
	}),
	(marked.walkTokens = function (e, t) {
		for (const n of e)
			switch ((t.call(marked, n), n.type)) {
				case 'table':
					for (const e of n.header) marked.walkTokens(e.tokens, t);
					for (const e of n.rows) for (const n of e) marked.walkTokens(n.tokens, t);
					break;
				case 'list':
					marked.walkTokens(n.items, t);
					break;
				default:
					marked.defaults.extensions &&
					marked.defaults.extensions.childTokens &&
					marked.defaults.extensions.childTokens[n.type]
						? marked.defaults.extensions.childTokens[n.type].forEach(function (e) {
								marked.walkTokens(n[e], t);
						  })
						: n.tokens && marked.walkTokens(n.tokens, t);
			}
	}),
