...
import { mapCanisterErrorToToastMessage, toToastError } from '../utils/error.utils';
import { getAccountIdentity, syncAccounts } from './accounts.services';
import { getIdentity } from './auth.services';
import { queryAndUpdate } from './utils.services';

export const listCanisters = async ({ clearBeforeQuery }: { clearBeforeQuery?: boolean }) => {
...
export const createCanister = async ({
	...
	try {
		...
		toastsShow(mapCanisterErrorToToastMessage(error, 'error.canister_creation_unknown'));
		return;
	}
};
...
export const topUpCanister = async ({
	...
	try {
		...
		toastsShow(mapCanisterErrorToToastMessage(error, 'error.canister_top_up_unknown'));
...
export const updateSettings = async ({
	...
	try {
		...
		toastsShow(mapCanisterErrorToToastMessage(error, 'error.canister_update_settings'));
		return { success: false };
	}
};

export const attachCanister = async (canisterId: Principal): Promise<{ success: boolean }> => {
	try {
		const identity = await getIdentity();
...
export const detachCanister = async (canisterId: Principal): Promise<{ success: boolean }> => {
	let success = false;
	try {
		const identity = await getIdentity();
...
export const routePathCanisterId = (path: string | undefined): string | undefined => {
 ...
 * @throws Error
 */
export const getCanisterDetails = async (canisterId: Principal): Promise<CanisterDetails> => {
	const identity = await getIdentity();
...
export const getIcpToCyclesExchangeRate = async (): Promise<bigint | undefined> => {
	try {
		const identity = await getIdentity();
