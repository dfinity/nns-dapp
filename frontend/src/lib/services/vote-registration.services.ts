...
import { Topic, Vote, type NeuronId, type ProposalId, type ProposalInfo } from '@dfinity/nns';
...
import { toastsError, toastsHide, toastsShow, toastsUpdate } from '../stores/toasts.store';
...
import { registerVoteErrorDetails, updateProposalVote } from '../utils/proposals.utils';
...
export const registerVotes = async ({
	...
	try {
		...
		const { successfullyVotedNeuronIds } = voteRegistrationByProposal(proposalId);
		...
		const updatedProposalInfo = await updateAfterVoteRegistration(proposalInfo.id as ProposalId);
...
const createRegisterVotesToast = ({
	...
	const status = replacePlaceholders($i18n.proposal_detail__vote.vote_status_registering, {
		$completed: `0`,
		$amount: `${totalNeurons}`
	});
...
const neuronRegistrationComplete = ({
	...
	const { vote, proposalInfo, neuronIds } = voteRegistrationByProposal(proposalId);
	const $definedNeuronsStore = get(definedNeuronsStore);
	const originalNeuron = $definedNeuronsStore.find(({ neuronId: id }) => id === neuronId);
	...
	updateVoteRegistrationToastMessage({
		...
		successfullyVotedNeuronIds: voteRegistrationByProposal(proposalId).successfullyVotedNeuronIds,
		vote
	});
};
...
const processRegisterVoteErrors = ({
	...
	const rejectedResponses = registerVoteResponses.filter((response: PromiseSettledResult<void>) => {
		...
		const hasAlreadyVoted: boolean =
			'reason' in response &&
			response.reason?.detail?.error_message === 'Neuron already voted on proposal.';
...
const updateAfterVoteRegistration = async (proposalId: ProposalId): Promise<ProposalInfo> => {
