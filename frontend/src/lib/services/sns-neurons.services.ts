...
import { SnsNeuronPermissionType, type SnsNeuron, type SnsNeuronId } from '@dfinity/sns';
...
import { snsNeuronsStore, type ProjectNeuronStore } from '../stores/sns-neurons.store';
...
export const loadSnsNeurons = async (rootCanisterId: Principal): Promise<void> => {
...
): ProjectNeuronStore | undefined => get(snsNeuronsStore)[rootCanisterId.toText()];
...
export const getSnsNeuron = async ({
	...
	rootCanisterId: Principal;
	forceFetch?: boolean;
	onLoad: ({ certified, neuron }: { certified: boolean; neuron: SnsNeuron }) => void;
	onError?: ({ certified, error }: { certified: boolean; error: unknown }) => void;
}): Promise<void> => {
	if (!forceFetch) {
