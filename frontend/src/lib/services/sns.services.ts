import { Topic, type AccountIdentifier, type ProposalInfo, type TokenAmount } from '@dfinity/nns';
...
import { snsProposalsStore, snsQueryStore, snsSwapCommitmentsStore } from '../stores/sns.store';
...
import { commitmentExceedsAmountLeft, validParticipation } from '../utils/projects.utils';
...
export const loadSnsSummary = async ({
	...
	queryAndUpdate<[QuerySnsMetadata | undefined, QuerySnsSwapState | undefined], unknown>({
		request: ({ certified, identity }) =>
			Promise.all([
				querySnsMetadata({
					rootCanisterId,
		...
		onLoad: ({ response: data }) => snsQueryStore.updateData({ data, rootCanisterId }),
		onError: ({ error: err, certified }) => {
			console.error(err);
...
export const loadSnsSwapCommitments = (): Promise<void> => {
	...
	return queryAndUpdate<SnsSwapCommitment[], unknown>({
		request: ({ certified, identity }) => querySnsSwapCommitments({ certified, identity }),
...
export const getSwapAccount = async (swapCanisterId: Principal): Promise<AccountIdentifier> => {
...
const getProjectFromStore = (rootCanisterId: Principal): SnsFullProject | undefined =>
	get(projectsStore)?.find(({ rootCanisterId: id }) => id.toText() === rootCanisterId.toText());
