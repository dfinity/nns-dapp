...
import { queryProposal, queryProposalPayload, queryProposals } from '../api/proposals.api';
import {
...
import { excludeProposals, proposalsHaveSameIds } from '../utils/proposals.utils';
...
export const listProposals = async ({
	...
	loadFinished: (params: { paginationOver: boolean; certified: boolean | undefined }) => void;
}): Promise<void> => {
...
const findProposals = async ({
	...
	return queryAndUpdate<ProposalInfo[], unknown>({
		request: ({ certified }) => queryProposals({ beforeProposal, identity, filters, certified }),
		onLoad: ({ response: proposals, certified }) => {
			if (certified === false) {
...
export const loadProposal = async ({
	...
	const catchError: QueryAndUpdateOnError<Error | unknown> = (erroneusResponse) => {
		...
		if (silentErrorMessages !== true && !skipUpdateErrorHandling) {
			...
			toastsShow({
				...
				level: 'error',
				detail: `id: "${proposalId}"${details === undefined ? '' : `. ${details}`}`
			});
		}
...
const getProposal = async ({
	...
	return queryAndUpdate<ProposalInfo | undefined, unknown>({
		request: ({ certified }) => queryProposal({ proposalId, identity, certified }),
		onLoad,
		onError,
		strategy,
