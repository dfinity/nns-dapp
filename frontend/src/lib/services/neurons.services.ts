...
import { ICPToken, TokenAmount, Topic, type NeuronId, type NeuronInfo } from '@dfinity/nns';
...
import { CannotBeMerged, NotAuthorizedNeuronError, NotFoundError } from '../types/neurons.errors';
import { assertEnoughAccountFunds, isAccountHardwareWallet } from '../utils/accounts.utils';
...
export const getNeuronFromStore = (neuronId: NeuronId): NeuronInfo | undefined =>
	get(definedNeuronsStore).find((neuron) => neuron.neuronId === neuronId);

// Exported to be tested
export const getIdentityOfControllerByNeuronId = async (neuronId: NeuronId): Promise<Identity> => {
	const { neuron, identity } = await getIdentityAndNeuronHelper(neuronId);

	if (neuron.fullNeuron === undefined || neuron.fullNeuron.controller === undefined) {
	...
	const neuronIdentity = await getAccountIdentityByPrincipal(neuron.fullNeuron.controller);
...
export const stakeNeuron = async ({
	...
	try {
		...
		if (isAccountHardwareWallet(account) && 'flagUpcomingStakeNeuron' in accountIdentity) {
...
const getAndLoadNeuron = async (neuronId: NeuronId) => {
	...
	if (!userAuthorizedNeuron(neuron)) {
		throw new NotAuthorizedNeuronError(`User not authorized to access neuron ${neuronId}`);
	}
...
export const updateDelay = async ({
	...
	try {
		const neuronIdentity: Identity = await getIdentityOfControllerByNeuronId(neuronId);
...
export const toggleCommunityFund = async (neuron: NeuronInfo): Promise<NeuronId | undefined> => {
	try {
		const identity: Identity = await getIdentityOfControllerByNeuronId(neuron.neuronId);
...
export const mergeNeurons = async ({
	...
	let success = false;
	try {
		const { neuron: neuron1 } = await getIdentityAndNeuronHelper(sourceNeuronId);
		const { neuron: neuron2 } = await getIdentityAndNeuronHelper(targetNeuronId);
		const { isValid, messageKey } = canBeMerged([neuron1, neuron2]);
		if (!isValid) {
			throw new CannotBeMerged(translate({ labelKey: messageKey ?? 'error.governance_error' }));
		}
		const identity: Identity = await getIdentityOfControllerByNeuronId(targetNeuronId);
...
export const addHotkey = async ({
	...
	try {
		const identity: Identity = await getIdentityOfControllerByNeuronId(neuronId);
...
export const removeHotkey = async ({
	...
	try {
		...
		if (removed && err instanceof NotAuthorizedNeuronError) {
			...
			const currentIdentityPrincipal = (await getIdentity()).getPrincipal().toText();
			// This happens when a user removes itself from the hotkeys.
			return principalString === currentIdentityPrincipal ? neuronId : undefined;
		}
...
export const startDissolving = async (neuronId: NeuronId): Promise<NeuronId | undefined> => {
	try {
		const identity: Identity = await getIdentityOfControllerByNeuronId(neuronId);
...
export const stopDissolving = async (neuronId: NeuronId): Promise<NeuronId | undefined> => {
	try {
		const identity: Identity = await getIdentityOfControllerByNeuronId(neuronId);
...
export const removeFollowee = async ({
	...
	const newFollowees: NeuronId[] = topicFollowees.filter((id) => id !== followee);
	await setFolloweesHelper({
		neuron,
		topic,
		followees: newFollowees
