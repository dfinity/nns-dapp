...
import { getAccountByPrincipal, getAccountFromStore } from '../utils/accounts.utils';
...
export const addSubAccount = async ({ name }: { name: string }): Promise<void> => {
	try {
...
export const routePathAccountIdentifier = (
	...
	return {
		accountIdentifier:
			accountIdentifier !== undefined && accountIdentifier !== '' ? accountIdentifier : undefined
	};
};
...
export const getAccountTransactions = async ({
	...
	queryAndUpdate<Transaction[], unknown>({
		...
		onLoad: ({ response: transactions }) => onLoad({ accountIdentifier, transactions }),
		onError: ({ error: err, certified }) => {
			console.error(err);
