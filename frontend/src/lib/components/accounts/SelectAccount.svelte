...
<div class="wizard-list" class:disabled={disableSelection} data-tid="select-account-screen">
	{#if mainAccount}
		{#if showTitle}
		...
		{#each subAccounts as subAccount}
			<AccountCard role="button" on:click={() => chooseAccount(subAccount)} account={subAccount}
		...
		{#if !hideHardwareWalletAccounts}
			{#each hardwareWalletAccounts as hardwareWalletAccount}
				<AccountCard
					...
					account={hardwareWalletAccount}>{hardwareWalletAccount.name}</AccountCard
				>
			{/each}
		{/if}
	{:else}
