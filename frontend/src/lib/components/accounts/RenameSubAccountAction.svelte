<script lang="ts">
	...
	const { store } = getContext<SelectedAccountContext>(SELECTED_ACCOUNT_CONTEXT_KEY);
...
<form on:submit|preventDefault={createNewSubAccount} class="wizard-wrapper">
	...
	<FooterModal>
		<button class="secondary" type="button" on:click={() => dispatcher('nnsClose')}>
			{$i18n.core.cancel}
		</button>
		<button
			class="primary"
			...
			disabled={newAccountName.length === 0 || $busy || selectedAccount === undefined}
		>
