<script lang="ts">
	...
	export let connectionState: LedgerConnectionState = LedgerConnectionState.NOT_CONNECTED;

	export let ledgerIdentity: LedgerIdentity | undefined = undefined;

	const connect = async () =>
		await connectToHardwareWalletProxy(({ ledgerIdentity: identity, connectionState: state }) => {
	...
	$: connected =
		connectionState === LedgerConnectionState.CONNECTED && ledgerIdentity !== undefined;
</script>
