<script lang="ts">
	...
	import { IS_TESTNET, ROLLUP_WATCH } from '../../constants/environment.constants';

	const localstorageKey: string = 'nnsdapp-testnet-banner-display';

	let visible: boolean = JSON.parse(localStorage.getItem(localstorageKey) ?? 'true') as boolean;
