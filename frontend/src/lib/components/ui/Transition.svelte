<script lang="ts">
	...
	$: slideOffset = transition.diff === 0 ? 0 : transition.diff > 0 ? absolutOffset : -absolutOffset;
</script>

{#key transition}
	<div bind:clientWidth={absolutOffset} in:fly={{ x: slideOffset, duration: ANIMATION_DURATION }}>
		<slot />
	</div>
{/key}
