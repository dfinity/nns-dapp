<script lang="ts">
	...
	import { getSnsNeuronHotkeys, canIdentityManageHotkeys } from '../../utils/sns-neuron.utils';
	import CardInfo from '../ui/CardInfo.svelte';
	import Tooltip from '../ui/Tooltip.svelte';
	import Value from '../ui/Value.svelte';
	import AddSnsHotkeyButton from './actions/AddSnsHotkeyButton.svelte';

	const { reload, store }: SelectedSnsNeuronContext = getContext<SelectedSnsNeuronContext>(
		SELECTED_SNS_NEURON_CONTEXT_KEY
	);
	...
	$: neuronId = neuron?.id !== undefined ? fromDefinedNullable(neuron.id) : undefined;
	...
	$: canManageHotkeys =
		neuron !== undefined && neuron !== null
			...
			: false;
	let hotkeys: string[];
	$: hotkeys = neuron !== undefined && neuron !== null ? getSnsNeuronHotkeys(neuron) : [];
...
{#if neuron !== undefined && neuron !== null}
	<CardInfo testId="sns-hotkeys-card">
		<div class="title" slot="start">
			...
			{#if showTooltip}
				<Tooltip id="sns-hotkeys-info" text={$i18n.sns_neuron_detail.add_hotkey_tooltip}>
					<span>
						<IconInfo />
					</span>
				</Tooltip>
