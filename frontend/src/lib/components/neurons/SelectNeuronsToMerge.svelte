<script lang="ts">
	...
	const toggleNeuronId = (neuronId: NeuronId): void => {
		if (selectedNeuronIds.includes(neuronId)) {
			selectedNeuronIds = selectedNeuronIds.filter((selectedId) => selectedId !== neuronId);
			return;
		}
...
<div class="wrapper">
	...
	<FooterModal>
		<button on:click={() => dispatcher('nnsClose')} class="secondary">{$i18n.core.cancel}</button>
		<button
			on:click={confirmSelection}
			class="primary"
			data-tid="merge-neurons-confirm-selection-button"
			disabled={!isMaxSelection}>{$i18n.neurons.merge_neurons_modal_merge_button}</button
		>
	</FooterModal>
</div>
