<script lang="ts">
	...
	$: {
		...
		const mapToKnownNeuron = (neuronId: NeuronId): FolloweeData => {
			const knownNeuron = $knownNeuronsStore.find((currentNeuron) => currentNeuron.id === neuronId);
			...
			<ul>
				{#each followees as followee}
					<li data-tid="current-followee-item">
						...
						<button on:click={() => removeCurrentFollowee(followee.neuronId)}>x</button>
					</li>
				{/each}
			</ul>
			<div class="button-wrapper">
				<button class="primary" data-tid="open-new-followee-modal" on:click={openNewFolloweeModal}
