<script lang="ts">
	...
	import { SECONDS_IN_EIGHT_YEARS, SECONDS_IN_HALF_YEAR } from '../../constants/constants';
	...
	$: disableUpdate = delayInSeconds < SECONDS_IN_HALF_YEAR || delayInSeconds <= minDelayInSeconds;
...
<div class="wizard-wrapper wrapper">
	<div>
		...
		{#if neuron.state === NeuronState.Locked && neuron.dissolveDelaySeconds}
			...
			<p class="duration">
				{@html valueSpan(secondsToDuration(neuron.dissolveDelaySeconds))} - {$i18n.neurons.staked}
			</p>
		{/if}
	</div>
