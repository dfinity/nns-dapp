<script lang="ts">
	...
	import { convertIcpToTCycles, convertTCyclesToIcpNumber } from '../../utils/icp.utils';
	...
	$: enoughCycles = minimumCycles === undefined ? true : (amountCycles ?? 0) >= minimumCycles;
</script>
...
>
	...
	<FooterModal>
		<button
			...
			on:click={() => dispatcher('nnsBack')}>{$i18n.canisters.edit_source}</button
		>
		<button
			type="submit"
			class="primary"
