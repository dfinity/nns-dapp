			...
			{#if neuron.votingPower}
				...
				{#if neuron.fullNeuron?.cachedNeuronStake !== undefined}
					<Tooltip
						...
						text={replacePlaceholders($i18n.neuron_detail.voting_power_tooltip, {
							$stake: formatToken({
							...
							$ageMultiplier: ageMultiplier(Number(neuron.ageSeconds)).toFixed(2)
						})}
					>
						<span>
							<IconInfo />
		...
		<div class="buttons">
			{#if isControllable}
				...
				{:else if neuron.state === NeuronState.Dissolving || neuron.state === NeuronState.Locked}
					<DissolveActionButton neuronState={neuron.state} neuronId={neuron.neuronId} />
				{/if}
			{/if}
		</div>
		<div class="buttons">
