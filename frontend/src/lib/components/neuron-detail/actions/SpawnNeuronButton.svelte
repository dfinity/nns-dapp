...
{:else}
	<Tooltip
		...
		text={replacePlaceholders($i18n.neuron_detail.spawn_maturity_disabled_tooltip, {
			$amount: formatNumber(MIN_NEURON_STAKE / E8S_PER_ICP / SPAWN_VARIANCE_PERCENTAGE, {
				minFraction: 4,
				maxFraction: 4
			}),
...
{#if isOpen}
	<SpawnNeuronModal on:nnsClose={closeModal} {neuron} {controlledByHardwareWallet} />
{/if}
