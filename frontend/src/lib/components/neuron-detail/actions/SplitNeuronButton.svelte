<script lang="ts">
	...
	import { minNeuronSplittable, neuronCanBeSplit } from '../../../utils/neuron.utils';
...
{#if splittable}
	<button on:click={openModal} class="primary">{$i18n.neuron_detail.split_neuron}</button>
{:else}
	<Tooltip
	...
	>
		<button on:click={openModal} class="primary" disabled>{$i18n.neuron_detail.split_neuron}</button
		>
	</Tooltip>
{/if}
