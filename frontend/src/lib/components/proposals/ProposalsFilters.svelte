<script lang="ts">
	...
	const openModal = (filters: ProposalsFilterModalProps) => (modalFilters = filters);
...
<div class={`filters ${VOTING_UI}`}>
	...
	<Checkbox
		...
		text="block"
		selector="hide-unavailable-proposals">{$i18n.voting.hide_unavailable_proposals}</Checkbox
	>
</div>

<ProposalsFilterModal props={modalFilters} on:nnsClose={() => (modalFilters = undefined)} />
