<script lang="ts">
	...
	import { filteredProposals, uiProposals } from '../../derived/proposals.derived';
	...
	$: proposalInfo,
		(() => {
			...
			previousProposal = [...$filteredProposals.proposals]
				.reverse()
				.find(
					({ id }) => proposalInfo?.id !== undefined && id !== undefined && id > proposalInfo.id
				);
	...
	$: lastProposal =
		proposalInfo !== undefined && nextProposal === undefined && previousProposal === undefined;
</script>
...
{#if $uiProposals.proposals.length > 0 && !lastProposal}
	<div role="toolbar">
		<button
			...
			on:click={previous}
			class:hidden={proposalInfo !== undefined && previousProposal === undefined}
