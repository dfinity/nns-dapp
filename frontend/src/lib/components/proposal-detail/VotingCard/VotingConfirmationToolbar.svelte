<script lang="ts">
	...
	import { mapProposalInfo, selectedNeuronsVotingPower } from '../../../utils/proposals.utils';
	...
	$: disabled =
		$votingNeuronSelectStore.selectedIds.length === 0 || $busy || voteRegistration !== undefined;
...
<div role="toolbar" class={`${layout}`} data-tid="voting-confirmation-toolbar">
	<button data-tid="vote-yes" {disabled} on:click={showAdoptConfirmation} class="success">
	...
	<button data-tid="vote-no" {disabled} on:click={showRejectConfirmation} class="danger">
