<script lang="ts">
	...
	const toggleSelection = (neuronId: bigint) => votingNeuronSelectStore.toggleSelection(neuronId);
</script>
			...
			<li>
				...
				>
					<span
						...
						aria-label={replacePlaceholders($i18n.proposal_detail__vote.cast_vote_neuronId, {
					...
					<span
						...
						aria-label={replacePlaceholders($i18n.proposal_detail__vote.cast_vote_votingPower, {
							$votingPower: formatVotingPower(getVotingPower({ neuron, proposal: proposalInfo }))
						})}>{`${formatVotingPower(getVotingPower({ neuron, proposal: proposalInfo }))}`}</span
					>
				</Checkbox>
			</li>
		{/each}
