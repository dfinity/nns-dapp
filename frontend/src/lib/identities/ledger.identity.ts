...
export class LedgerIdentity extends SignIdentity {
	 ...
	 */
	public override async transformRequest(request: HttpAgentRequest): Promise<unknown> {
		/**
		 ...
		 */
		const prepareCborForLedger = (request: ReadRequest | CallRequest): ArrayBuffer =>
			Cbor.encode({ content: request });
