<script lang="ts">
	...
	import { getSwapAccount, participateInSwap } from '../../../services/sns.services';
	...
	$: (async () => {
		destinationAddress =
			$projectDetailStore.summary?.swapCanisterId !== undefined
				? (await getSwapAccount($projectDetailStore.summary?.swapCanisterId)).toHex()
				: undefined;
	})();
	...
	$: minCommitment = TokenAmount.fromE8s({
		amount: userHasParticipatedToSwap ? BigInt(0) : params.min_participant_icp_e8s,
		token: ICPToken
	});
...
{#if destinationAddress !== undefined}
	...
	>
		<svelte:fragment slot="title">{title ?? $i18n.sns_project_detail.participate}</svelte:fragment>
		<AdditionalInfoForm
