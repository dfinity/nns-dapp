<script lang="ts">
	...
	import { getIcpToCyclesExchangeRate, topUpCanister } from '../../services/canisters.services';
	...
	const { store, reloadDetails }: CanisterDetailsContext = getContext<CanisterDetailsContext>(
		CANISTER_DETAILS_CONTEXT_KEY
	);
	...
	const onSelectAccount = ({ detail }: CustomEvent<{ selectedAccount: Account }>) => {
	...
	const addCycles = async () => {
		...
		if (amount === undefined || account === undefined || canisterId === undefined) {
			toastsError({
...
<LegacyWizardModal {steps} bind:currentStep bind:this={modal} on:nnsClose>
	...
	<svelte:fragment>
		{#if currentStep?.name === 'SelectAccount'}
			<SelectAccount hideHardwareWalletAccounts on:nnsSelectAccount={onSelectAccount} />
		{/if}
			...
			>
				<p>
					{@html replacePlaceholders($i18n.canisters.transaction_fee, {
						$amount: valueSpan(formattedTransactionFeeICP($mainTransactionFeeStore))
					})}
				</p>
				<div>
					<div>
