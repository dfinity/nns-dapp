<script lang="ts">
	...
	$: filtersValues = filters
		? enumValues(filters).filter((value) => value !== PROPOSAL_FILTER_UNSPECIFIED_VALUE)
		: [];
	...
	const applyFilterChange = (filter: Topic | ProposalRewardStatus | ProposalStatus) =>
		(selectedFilters = selectedFilters.includes(filter)
			? selectedFilters.filter(
					(activeTopic: Topic | ProposalRewardStatus | ProposalStatus) => activeTopic !== filter
			  )
			: [...selectedFilters, filter]);
	...
	const updateProposalStoreFilters = () => {
		switch (category) {
			case 'topics':
				proposalsFiltersStore.filterTopics(selectedFilters as unknown as Topic[]);
				return;
			case 'rewards':
				proposalsFiltersStore.filterRewards(selectedFilters as unknown as ProposalRewardStatus[]);
				return;
			case 'status':
				proposalsFiltersStore.filterStatus(selectedFilters as unknown as ProposalStatus[]);
				return;
		}
	};
		...
		<div class="filters">
			{#each filtersValues as key (key)}
				<Checkbox
					...
					on:nnsChange={() => onChange(key)}>{$i18n[category][filters[key]]}</Checkbox
				>
			{/each}
		</div>
	{/if}
