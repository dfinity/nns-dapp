<script lang="ts">
	...
	$: newNeuron = $definedNeuronsStore.find(({ neuronId }) => neuronId === newNeuronId);
	...
	const chooseAccount = async ({ detail }: CustomEvent<{ selectedAccount: Account }>) => {
	...
	const onNeuronCreated = async ({ detail }: CustomEvent<{ neuronId: NeuronId }>) => {
