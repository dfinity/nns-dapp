...
<LegacyWizardModal {steps} bind:currentStep bind:this={modal} on:nnsClose>
	<span slot="title" data-tid="merge-maturity-neuron-modal"
		>{currentStep?.title ?? $i18n.neuron_detail.merge_maturity_modal_title}</span
	>
	...
	{:else if currentStep.name === 'ConfirmMerge'}
		<ConfirmActionScreen on:nnsConfirm={mergeNeuronMaturity} on:nnsCancel={modal.back}>
			<div class="confirm" slot="main-info">
				<h4>{$i18n.neuron_detail.merge_maturity_confirmation_q}</h4>
				<p class="confirm-answer">
					{@html replacePlaceholders($i18n.neuron_detail.merge_maturity_confirmation_a, {
			...
			<svelte:fragment slot="button-content">{$i18n.core.confirm}</svelte:fragment>
