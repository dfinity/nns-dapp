<script lang="ts">
	...
	const steps: Steps = controlledByHardwareWallet ? hardwareWalletSteps : nnsDappAccountSteps;
	...
	const spawnNeuronFromMaturity = async () => {
		...
		const newNeuronId = await spawnNeuron({
			...
			percentageToSpawn: controlledByHardwareWallet ? undefined : percentageToSpawn
		});
...
<LegacyWizardModal {steps} bind:currentStep on:nnsClose>
	<svelte:fragment slot="title"
		>{currentStep?.title ?? $i18n.neuron_detail.spawn_maturity_modal_title}</svelte:fragment
	>
