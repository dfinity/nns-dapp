<script lang="ts">
	...
	const { store: storeAccount } = getContext<SelectedAccountContext>(SELECTED_ACCOUNT_CONTEXT_KEY);
	...
	const context: HardwareWalletNeuronsContext = getContext<HardwareWalletNeuronsContext>(
		HARDWARE_WALLET_NEURONS_CONTEXT_KEY
	);
...
<ConfirmationModal on:nnsClose on:nnsConfirm={addCurrentUserToHotkey}>
	...
	<p>
		{replacePlaceholders($i18n.accounts.hardware_wallet_add_hotkey_text_neuron, {
			$neuronId: neuronId.toString()
		})}
	</p>

	<p>
		{replacePlaceholders($i18n.accounts.hardware_wallet_add_hotkey_text_principal, {
