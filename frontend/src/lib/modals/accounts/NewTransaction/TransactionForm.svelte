...
<form on:submit|preventDefault={goNext} class="wrapper" data-tid="transaction-step-1">
	<div class="select-account">
		{#if selectedAccount !== undefined}
			<KeyValuePair>
				<span slot="key" class="label">{$i18n.accounts.source}</span>
				<AmountDisplay slot="value" singleLine amount={selectedAccount?.balance} />
			</KeyValuePair>
		{/if}
		{#if canSelectSource}
	...
	<FooterModal>
		<button class="secondary" data-tid="transaction-button-cancel" type="button" on:click={close}
