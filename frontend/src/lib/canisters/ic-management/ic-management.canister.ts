import { getManagementCanister, type ManagementCanisterRecord } from '@dfinity/agent';
...
const wrapWithArray = <T>(value: T | undefined): [T] | [] => (value === undefined ? [] : [value]);
...
export class ICManagementCanister {
	 ...
	 */
	public getCanisterDetails = async (canisterId: Principal): Promise<CanisterDetails> => {
		try {
			const rawResponse: CanisterStatusResponse = await this.service.canister_status({
	...
	public updateSettings = async ({
		canisterId,
		settings: { controllers, freezingThreshold, memoryAllocation, computeAllocation }
	}: {
		...
		try {
			...
			await this.service.update_settings({
				canister_id: canisterId,
				settings: {
					controllers: wrapWithArray(controllers?.map((c) => Principal.fromText(c))),
