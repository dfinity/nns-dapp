...
export class NNSDappCanister {
	 ...
	 * @returns Promise<void>
	 */
	public async getAccount({ certified }: { certified: boolean }): Promise<AccountDetails> {
		const response: GetAccountResponse = await this.getNNSDappService(certified).get_account();
	...
	public async createSubAccount({
		...
		const response: CreateSubAccountResponse = await this.certifiedService.create_sub_account(
			subAccountName
		);
		...
		if ('SubAccountLimitExceeded' in response) {
			...
			throw new SubAccountLimitExceededError('error__account.create_subaccount_limit_exceeded');
		}
	...
	public async registerHardwareWallet(request: RegisterHardwareWalletRequest): Promise<void> {
		...
		if ('AccountNotFound' in response && response.AccountNotFound === null) {
			throw new AccountNotFoundError('error__attach_wallet.register_hardware_wallet');
		}

		if ('NameTooLong' in response && response.NameTooLong === null) {
			throw new NameTooLongError('error__attach_wallet.create_hardware_wallet_too_long', {
		...
		if (
			...
			throw new HardwareWalletAttachError('error__attach_wallet.already_registered');
		}
		...
		if (
			...
			throw new HardwareWalletAttachError('error__attach_wallet.limit_exceeded');
		}
	}

	public renameSubAccount = async (request: RenameSubAccountRequest): Promise<void> => {
		const response: RenameSubAccountResponse = await this.certifiedService.rename_sub_account(
			request
		);

		if ('AccountNotFound' in response && response.AccountNotFound === null) {
			throw new AccountNotFoundError('error__account.rename_account_not_found', {
				$account_identifier: request.account_identifier
			});
		}

		if ('SubAccountNotFound' in response && response.SubAccountNotFound === null) {
	...
	public attachCanister = async ({
		...
		if ('CanisterAlreadyAttached' in response && response.CanisterAlreadyAttached === null) {
			throw new CanisterAlreadyAttachedError('error__canister.already_attached', {
		...
		if ('CanisterLimitExceeded' in response && response.CanisterLimitExceeded === null) {
	...
	public async getProposalPayload({ proposalId }: { proposalId: ProposalId }): Promise<object> {
	...
	public async addPendingNotifySwap(request: AddPendingNotifySwapRequest): Promise<void> {
		const response = await this.certifiedService.add_pending_notify_swap(request);
		if ('Ok' in response) {
			return;
		}
		...
		// Edge case
		throw new Error(`Unknown response for add_pending_notify_swap ${JSON.stringify(response)}`);
	}
}
