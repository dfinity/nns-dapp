<script lang="ts">
	...
	import { loadProposal, routePathProposalId } from '../services/proposals.services';
	...
	const neuronsStoreReady = (): boolean => {
		...
		return $neuronsStore.neurons !== undefined && $neuronsStore.certified === true;
	};
	...
	$: routeProposalId,
		(async () => {
			...
			if (storeProposalId !== routeProposalId.proposalId || storeProposalId === undefined) {
	...
	const goBack = () => {
		...
		routeStore.navigate({
			path: referrerPath === AppPath.Launchpad ? AppPath.Launchpad : AppPath.Proposals
		});
	};
	...
	$: layoutTitleStore.set(
		`${$i18n.proposal_detail.title}${
			$selectedProposalStore.proposalId !== undefined ? ` ${$selectedProposalStore.proposalId}` : ''
		}`
	);
</script>
