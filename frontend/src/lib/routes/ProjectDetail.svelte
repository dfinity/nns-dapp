<script lang="ts">
	...
	const reload = async () => {
		...
		await Promise.all([loadSummary(rootCanisterId), loadSwapState(rootCanisterId)]);
	};
	...
	const mapProjectDetail = (rootCanisterId: string) => {
		...
		if ($snsSummariesStore.length === 0 || isNullish($snsSwapCommitmentsStore)) {
			return;
		}
		...
		$projectDetailStore.summary =
			rootCanisterId !== undefined
				? $snsSummariesStore.find(
						({ rootCanisterId: rootCanister }) => rootCanister?.toText() === rootCanisterId
				  )
				: undefined;
