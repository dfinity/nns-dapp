<script lang="ts">
	...
	const canistersStoreReady = (): boolean => {
		...
		return $canistersStore.canisters !== undefined && $canistersStore.certified === true;
	};
	...
	$: routeCanisterId,
		...
		(() => {
			...
			const storeCanister: CanisterInfo | undefined = $selectedCanisterStore.info;
			...
			if (storeCanister !== selectedCanister) {
				...
				const sameCanister: boolean =
					...
					storeCanister?.canister_id.toHex() === selectedCanister.canister_id.toText();
	...
	$: ({ details: canisterDetails, info: canisterInfo } = $selectedCanisterStore);
</script>
...
<Footer>
	<Toolbar>
		<button
			...
			disabled={canisterInfo === undefined || $busy}>{$i18n.canister_detail.add_cycles}</button
		>
	</Toolbar>
</Footer>
