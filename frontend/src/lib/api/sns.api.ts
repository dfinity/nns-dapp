...
import type { QueryRootCanisterId, QuerySnsMetadata, QuerySnsSwapState } from '../types/sns.query';
...
export const queryAllSnsMetadata = async ({
	...
	const snsWrappers: SnsWrapper[] = [
		...(
			(await wrappers({ identity, certified })) ?? new Map<QueryRootCanisterId, SnsWrapper>()
		).values()
	];
...
export const querySnsSwapStates = async ({
	...
	logWithTimestamp(`Listing all deployed Sns swap states certified:${certified} call...`);
	...
	logWithTimestamp(`Listing all deployed Sns swap states certified:${certified} done.`);
...
export const querySnsSwapState = async ({
	...
	logWithTimestamp(`Getting Sns ${rootCanisterId} swap state certified:${certified} call...`);
	...
	logWithTimestamp(`Getting Sns ${rootCanisterId} swap state certified:${certified} done.`);
...
export const querySnsSwapCommitments = async ({
	...
	logWithTimestamp(`Listing all deployed Sns swap commitments certified:${certified} call...`);
	...
	logWithTimestamp(`Listing all deployed Sns swap commitments certified:${certified} done.`);
...
export const querySnsSwapCommitment = async ({
	...
	logWithTimestamp(`Getting Sns ${rootCanisterId} swap commitment certified:${certified} call...`);
	...
	logWithTimestamp(`Getting Sns ${rootCanisterId} swap commitment certified:${certified} done.`);
