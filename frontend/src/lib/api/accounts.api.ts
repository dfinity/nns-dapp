...
export const loadAccounts = async ({
	...
	// Helper
	const getAccountBalance = async (identifierString: string): Promise<TokenAmount> => {
	...
	const [main, subAccounts, hardwareWallets] = await Promise.all([
		...
		Promise.all(mainAccount.hardware_wallet_accounts.map(mapAccount('hardwareWallet')))
	]);
...
export const getTransactions = async ({
	...
	logWithTimestamp(`Loading Transactions ${hashCode(accountIdentifier)} call...`);
	...
	logWithTimestamp(`Loading Transactions ${hashCode(accountIdentifier)} complete.`);
...
export const renameSubAccount = async ({
	...
	logWithTimestamp(`Renaming SubAccount ${hashCode(subAccountIdentifier)} call...`);
	...
	logWithTimestamp(`Renaming SubAccount ${hashCode(subAccountIdentifier)} complete.`);
};
