...
let snsUpdateWrappers: Promise<Map<QueryRootCanisterId, SnsWrapper>> | undefined;
...
const listSnses = async ({
	...
	return snses.reduce(
		(acc: Principal[], { root_canister_id }: DeployedSns) => [...acc, ...root_canister_id],
		[]
	);
};
...
const loadSnsWrappers = async ({
	...
	const error: boolean = results.find(({ status }) => status === 'rejected') !== undefined;
	if (error) {
...
const initWrappers = async ({
	...
	new Map(
		(await loadSnsWrappers({ identity, certified })).map((wrapper: SnsWrapper) => [
...
export const wrapper = async ({
	...
	const snsWrapper: SnsWrapper | undefined = (await wrappers({ identity, certified })).get(
		rootCanisterId
	);
