...
import type { E8s, KnownNeuron, NeuronId, NeuronInfo, TokenAmount, Topic } from '@dfinity/nns';
...
export const queryNeuron = async ({
	...
	logWithTimestamp(`Querying Neuron(${hashCode(neuronId)}) certified:${certified} call...`);
	...
	logWithTimestamp(`Querying Neuron(${hashCode(neuronId)}) certified:${certified} complete.`);
	return response;
};
...
export const increaseDissolveDelay = async ({
	...
	logWithTimestamp(
		`Increasing Dissolve Delay(${hashCode(neuronId)}, ${hashCode(dissolveDelayInSeconds)}) call...`
	);
	const { canister } = await governanceCanister({ identity });
...
export const removeHotkey = async ({
	...
	logWithTimestamp(`Remove hotkey (for neuron ${hashCode(neuronId)}) complete.`);
};
...
export const mergeNeurons = async ({
	...
	logWithTimestamp(
		`Merging neurons (${hashCode(sourceNeuronId)}, ${hashCode(targetNeuronId)}) call...`
	);
	const { canister } = await governanceCanister({ identity });
	...
	logWithTimestamp(
		`Merging neurons (${hashCode(sourceNeuronId)}, ${hashCode(targetNeuronId)}) complete.`
	);
};
...
export const claimOrRefreshNeuron = async ({
	...
	logWithTimestamp(`ClaimingOrRefreshing Neurons (${hashCode(neuronId)}) call...`);
	const { canister } = await governanceCanister({ identity });
	...
	logWithTimestamp(`ClaimingOrRefreshing Neurons (${hashCode(neuronId)}) complete.`);
	return response;
};
