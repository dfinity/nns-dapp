...
import type { ListProposalsResponse, NeuronId, ProposalId, ProposalInfo, Vote } from '@dfinity/nns';
...
export const queryProposal = async ({
	...
	logWithTimestamp(`Querying Proposal (${hashCode(proposalId)}) certified:${certified} call...`);
	...
	logWithTimestamp(`Querying Proposal (${hashCode(proposalId)}) certified:${certified} complete.`);
...
export const queryProposalPayload = async ({
	...
	logWithTimestamp(`Loading Proposal Payload ${hashCode(proposalId)} complete.`);
...
export const registerVote = async ({
	...
	logWithTimestamp(`Registering Vote (${hashCode(proposalId)}, ${hashCode(neuronId)}) call...`);
	...
	logWithTimestamp(`Registering Vote (${hashCode(proposalId)}, ${hashCode(neuronId)}) complete.`);
};
