...
export const getIcpToCyclesExchangeRate = async (identity: Identity): Promise<bigint> => {
...
const pollingLimit = (error: Error): boolean => error instanceof PollingLimitExceededError;
const notProcessingError = (error: Error): boolean => !(error instanceof ProcessingError);
