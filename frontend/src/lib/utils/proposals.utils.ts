...
import { NnsFunction, ProposalRewardStatus, ProposalStatus, Topic, Vote } from '@dfinity/nns';
...
export const lastProposalId = (proposalInfos: ProposalInfo[]): ProposalId | undefined => {
...
export const proposalActionFields = (proposal: Proposal): [string, unknown][] => {
...
const matchFilters = ({
	...
	const { topic: proposalTopic, status: proposalStatus, rewardStatus } = proposalInfo;
...
export const hasMatchingProposals = ({
	...
	return (
		proposals.find(
			(proposalInfo: ProposalInfo) => !hideProposal({ proposalInfo, filters, neurons })
		) !== undefined
	);
};
...
export const getVotingBallot = ({
	...
	neuronId: bigint;
	proposalInfo: ProposalInfo;
}): Ballot | undefined => ballots.find((ballot) => ballot.neuronId === neuronId);
...
export const preserveNeuronSelectionAfterUpdate = ({
	...
	const newNeuronsSelection = Array.from(newIds).filter((id) => oldIds.has(id) === false);
...
export const replaceAndConcatenateProposals = ({
	...
	const updatedProposals = oldProposals.map(
		(stateProposal) => newProposals.find(({ id }) => stateProposal.id === id) ?? stateProposal
	);

	const newStateProposalsIds: (ProposalId | undefined)[] = updatedProposals.map(({ id }) => id);
...
export const replaceProposals = ({
	...
	oldProposals.map(
		(oldProposal) => newProposals.find(({ id }) => id === oldProposal.id) ?? oldProposal
	);
...
export const mapProposalInfo = (proposalInfo: ProposalInfo): ProposalInfoMap => {
	const {
		proposal,
		proposer,
		id,
	...
	return {
		...
		executed: executedTimestampSeconds > 0 ? executedTimestampSeconds : undefined,
...
const mapProposalType = (
	...
	const { actions, actions_description, nns_functions, nns_functions_description } = get(i18n);
...
const votingPeriodEndFallback = ({ proposalTimestampSeconds, topic }: ProposalInfo): Date => {
	const durationInSeconds: number = [Topic.ManageNeuron, Topic.ExchangeRate].includes(topic)
		? SHORT_VOTING_PERIOD_SECONDS
		: WAIT_FOR_QUIET_THRESHOLD_SECONDS;

	return new Date(Number(proposalTimestampSeconds) * 1000 + durationInSeconds * 1000);
};
...
export const updateProposalVote = ({
	...
	return {
		...proposalInfo,
		ballots: [
			...proposalInfo.ballots.filter(({ neuronId }) => neuronId !== neuron.neuronId),
			votedBallot
		],
		latestTally: {
...
const registerVoteErrorReason = (
	...
	const reason = result.reason instanceof Error ? errorToString(result.reason) : undefined;
	// detail text
	return replacePlaceholders(get(i18n).error.register_vote_neuron, {
		$neuronId: neuronId.toString(),
		$reason: reason === undefined || reason?.length === 0 ? get(i18n).error.fail : reason
	});
};
