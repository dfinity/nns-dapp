...
import type { CanisterDetails, Transaction } from '../canisters/nns-dapp/nns-dapp.types';
...
const anonymiseAvailability = (value: unknown): 'yes' | 'no' => (nonNullish(value) ? 'yes' : 'no');
...
export const cutAndAnonymize = async (
	...
	value === undefined
		? undefined
		: await anonymize(`${value}`.substring(0, Math.ceil(`${value}`.length * 0.7)));

export const anonymizeAmount = async (amount: bigint | undefined): Promise<bigint | undefined> =>
	amount === undefined
		? undefined
...
): Promise<{ [key in keyof Required<Followees>]: unknown } | undefined | null> => {
...
): Promise<{ [key in keyof Required<Account>]: unknown } | undefined | null> => {
...
): Promise<undefined | { [key in keyof Required<CanisterDetails>]: unknown }> => {
...
export const anonymizeTransaction = async ({
...
}): Promise<undefined | { [key in keyof Required<Transaction>]: unknown } | 'no account'> => {
	...
	const { isReceive, isSend, type, from, to, displayAmount, date } = mapTransaction({
		transaction,
		account
	});
...
const anonymizeBuyer = async ([buyer, state]): Promise<[string, SnsSwapBuyerState]> => [
	buyer,
	{
		icp: [
			{
				...state.icp[0],
				amount_e8s: (await anonymizeAmount(state.icp[0]?.amount_e8s)) ?? BigInt(0)
			}
		]
	}
];
...
export const anonymizeSnsSummary = async (
	...
	if (originalSummary !== undefined && originalSummary !== null) {
		const anonymizedBuyers = await mapPromises(originalSummary?.swap.buyers, anonymizeBuyer);
		return {
			...originalSummary,
				...
				icp:
					commitment !== undefined
						? [
								{
									...commitment,
									amount_e8s: (await anonymizeAmount(commitment.amount_e8s)) as bigint
								}
						  ]
						: []
			}
