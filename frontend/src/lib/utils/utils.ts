...
export const targetBlankLinkRenderer = (
	...
	`<a${href === null ? '' : ` target="_blank" rel="noopener noreferrer" href="${href}"`}${
 ...
 */
export const createChunks = <T>(elements: Array<T>, chunkSize = 10): Array<Array<T>> => {
 ...
 */
export const valueSpan = (text: string): string => `<span class="value">${text}</span>`;
