...
export const getAccountByPrincipal = ({
	...
	return hardwareWallets?.find(({ principal: hwPrincipal }) => hwPrincipal?.toText() === principal);
};
...
export const getAccountFromStore = ({
	...
	return hardwareWallets?.find((account: Account) => account.identifier === identifier);
};
