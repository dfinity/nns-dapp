...
export const sumTokenAmounts = (...amountTokens: TokenAmount[]): TokenAmount => {
	if (
		amountTokens.some((amountToken) => amountToken.token.symbol !== amountTokens[0].token.symbol)
	) {
		throw new Error('Token symbols must be equal');
	}
	return TokenAmount.fromE8s({
		amount: amountTokens.reduce<bigint>((acc, icp) => acc + icp.toE8s(), BigInt(0)),
...
export const getMaxTransactionAmount = ({
	...
	return Math.min(Number(maxAmount), Math.max(Number(balance - fee), 0)) / E8S_PER_ICP;
};
