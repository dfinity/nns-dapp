...
const getStakingPower = (params: Params) => {
  ...
  return {
    value: totalStakedUSD / totalValueUSD,
...
const getTotalVotingPower = (): bigint => {
  // @TODO MOCKED lastRewardEvent -> totalVotingPower
...
const getGenesisTimestampSeconds = (sns?: CachedSnsDto): number => {
  const MOCKED_SNS_GENESIS_TIMESTAMP_SECONDS: Record<string, number> = {
    // Alice
  ...
  if (sns) {
    ...
    const snsGenesisTimestamp =
      MOCKED_SNS_GENESIS_TIMESTAMP_SECONDS[sns.canister_ids.root_canister_id];
...
const getNnsRewardParams = (params: Params) => ({
  ...
  totalSupply: 534_809_202, // @TODO MOCKED
});
