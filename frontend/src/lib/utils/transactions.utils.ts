...
import type { AccountIdentifierString, Transaction } from '../canisters/nns-dapp/nns-dapp.types';
...
export const accountName = ({
	...
	mainName: string;
}): string => account?.name ?? (account?.type === 'main' ? mainName : account?.name ?? '');
...
export const transactionType = (transaction: Transaction): AccountTransactionType => {
	...
	throw new Error('Unknown TransactionType: ' + JSON.stringify(transactionType));
};
...
export const mapTransaction = ({
	...
	const useFee = toSelfTransaction === true ? false : showTransactionFee({ type, isReceive });
