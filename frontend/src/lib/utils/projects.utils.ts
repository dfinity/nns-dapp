...
import type { SnsSummary, SnsSummarySwap, SnsSwapCommitment } from '../types/sns';
...
export const filterCommittedProjects = (projects: SnsFullProject[] | undefined) =>
...
}: SnsSummarySwap): bigint | undefined => swap_due_timestamp_seconds - BigInt(nowInSeconds());
...
export const currentUserMaxCommitment = ({
	...
	const remainingProjectCommitment = swap.params.max_icp_e8s - derived.buyer_total_icp_e8s;
	const remainingUserCommitment =
		swap.params.max_participant_icp_e8s - (getCommitmentE8s(swapCommitment) ?? BigInt(0));
...
export const validParticipation = ({
	...
	const totalCommitment = (getCommitmentE8s(project.swapCommitment) ?? BigInt(0)) + amount.toE8s();
	if (commitmentTooLarge({ summary: project.summary, amountE8s: totalCommitment })) {
		return {
			valid: false,
		...
		return {
			...
			substitutions: {
				...
				$remainingCommitment: formatToken({
					value:
						project.summary.swap.params.max_icp_e8s - project.summary.derived.buyer_total_icp_e8s
				})
			}
		};
	}
