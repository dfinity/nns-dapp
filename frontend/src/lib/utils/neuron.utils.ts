...
import { DEFAULT_TRANSACTION_FEE_E8S, E8S_PER_ICP } from '../constants/icp.constants';
import {
	MAX_NEURONS_MERGED,
	MIN_NEURON_STAKE,
...
export const hasValidStake = (neuron: NeuronInfo): boolean =>
	...
	nonNullish(neuron.fullNeuron)
		? neuron.fullNeuron.cachedNeuronStake + neuron.fullNeuron.maturityE8sEquivalent >
		  BigInt(DEFAULT_TRANSACTION_FEE_E8S)
		: false;

export const dissolveDelayMultiplier = (delayInSeconds: number): number =>
	1 + 1 * (Math.min(delayInSeconds, SECONDS_IN_EIGHT_YEARS) / SECONDS_IN_EIGHT_YEARS);

export const getDissolvingTimeInSeconds = (neuron: NeuronInfo): bigint | undefined =>
	neuron.state === NeuronState.Dissolving &&
	neuron.fullNeuron?.dissolveState !== undefined &&
	'WhenDissolvedTimestampSeconds' in neuron.fullNeuron.dissolveState
		? neuron.fullNeuron.dissolveState.WhenDissolvedTimestampSeconds - BigInt(nowInSeconds())
		: undefined;

export const getSpawningTimeInSeconds = (neuron: NeuronInfo): bigint | undefined =>
...
export const ageMultiplier = (ageSeconds: number): number =>
	1 + 0.25 * (Math.min(ageSeconds, SECONDS_IN_FOUR_YEARS) / SECONDS_IN_FOUR_YEARS);
...
export const sortNeuronsByCreatedTimestamp = (neurons: NeuronInfo[]): NeuronInfo[] =>
	[...neurons].sort((a, b) => Number(b.createdTimestampSeconds - a.createdTimestampSeconds));
...
export const isNeuronControllable = ({
	...
	(fullNeuron.controller === identity?.getPrincipal().toText() ||
		getAccountByPrincipal({ principal: fullNeuron.controller, accounts }) !== undefined);
...
export const isHotKeyControllable = ({
	...
	fullNeuron?.hotKeys.find((hotkey) => hotkey === identity?.getPrincipal().toText()) !==
		undefined && fullNeuron.controller !== identity?.getPrincipal().toText();
 ...
 */
export const ballotsWithDefinedProposal = ({ recentBallots }: NeuronInfo): Required<BallotInfo>[] =>
	recentBallots.filter(({ proposalId }: BallotInfo) => proposalId !== undefined);

export const isValidInputAmount = ({ amount, max }: { amount?: number; max: number }): boolean =>
	amount !== undefined && amount > 0 && amount <= max;
...
export const isSpawning = (neuron: NeuronInfo): boolean => neuron.state === NeuronState.Spawning;
...
export const mapMergeableNeurons = ({
	...
	neurons
		...
		.map((neuron: NeuronInfo) => ({
			neuron,
			selected: selectedNeurons.map(({ neuronId }) => neuronId).includes(neuron.neuronId),
 ...
 * @returns boolean
 */
export const allHaveSameFollowees = (sortedFolloweesLists: NeuronId[][]): boolean =>
	new Set(sortedFolloweesLists.map((list) => list.join())).size === 1;

const sameManageNeuronFollowees = (neurons: NeuronInfo[]): boolean => {
	const fullNeurons: Neuron[] = neurons.map(({ fullNeuron }) => fullNeuron).filter(isDefined);
...
export const canBeMerged = (neurons: NeuronInfo[]): { isValid: boolean; messageKey?: string } => {
...
export const mapNeuronIds = ({
	...
	neuronIds
		.map((selectedId) => neurons.find(({ neuronId }) => neuronId === selectedId))
		.filter(isDefined);
...
export const checkInvalidState = <T>({
	...
	invalidStates
		.filter(({ stepName, isInvalid }) => stepName === currentStep?.name && isInvalid(args))
...
export const followeesByTopic = ({
	...
	neuron?.fullNeuron?.followees.find(({ topic: followedTopic }) => topic === followedTopic)
		?.followees;
...
const getRecentBallot = ({
	...
	neuron.recentBallots.find(({ proposalId: currentId }) => currentId === proposalId);
...
export const hasEnoughMaturityToMerge = ({
	...
	neuron: NeuronInfo;
	fee: number;
}): boolean => fullNeuron !== undefined && fullNeuron.maturityE8sEquivalent > minMaturityMerge(fee);

export const minNeuronSplittable = (fee: number): number => 2 * E8S_PER_ICP + fee;

export const neuronCanBeSplit = ({ neuron, fee }: { neuron: NeuronInfo; fee: number }): boolean =>
	neuronStake(neuron) >= BigInt(minNeuronSplittable(fee));
...
export const getNeuronById = ({
	...
	neuronId: NeuronId;
}): NeuronInfo | undefined => neuronsStore.neurons?.find((n) => n.neuronId === neuronId);
...
export const neuronVoting = ({
	...
	registrations.find(
		({ neuronIds, successfullyVotedNeuronIds }) =>
			neuronIds.includes(neuronId) && !successfullyVotedNeuronIds.includes(neuronId)
	) !== undefined;
