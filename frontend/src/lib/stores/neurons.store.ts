...
import { hasValidStake, sortNeuronsByCreatedTimestamp } from '../utils/neuron.utils';
	...
	return {
		...
		pushNeurons({ neurons, certified }: Required<NeuronsStore>) {
			update(({ neurons: oldNeurons }: NeuronsStore) => {
				...
				const newIds = new Set(filteredNewNeurons.map(({ neuronId }) => neuronId));
				return {
					neurons: [
		...
		replaceNeurons(neurons: NeuronInfo[]) {
			...
			update(({ neurons: oldNeurons, certified }: NeuronsStore) => {
				const newNeurons = new Map(neurons.map((neuron) => [neuron.neuronId, neuron]));
...
export const sortedNeuronStore: Readable<NeuronInfo[]> = derived(
	definedNeuronsStore,
	(initializedNeuronsStore) => sortNeuronsByCreatedTimestamp(initializedNeuronsStore)
);
