...
export const busy: Readable<boolean> = derived(busyStore, ($busyStore) => $busyStore.length > 0);

// Returns the newest message that was added to the store
export const busyMessageKey: Readable<string | undefined> = derived(
	busyStore,
	($busyStore) => $busyStore.reverse().find(({ labelKey }) => labelKey !== undefined)?.labelKey
);
