<script lang="ts">
	...
	const unsubscribeAuth: Unsubscriber = authStore.subscribe(async (auth: AuthStore) => {
		// TODO: sveltekit
	...
	const unsubscribeRoute: Unsubscriber = routeStore.subscribe(({ isKnownPath }) => {
		if (isKnownPath) {
			return;
		}
	...
	const unsubscribeVoteInProgress: Unsubscriber = voteRegistrationStore.subscribe(
		({ registrations }) => syncBeforeUnload(voteRegistrationActive(registrations))
	);
