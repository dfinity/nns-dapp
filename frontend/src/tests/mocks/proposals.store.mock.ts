...
import { ProposalRewardStatus, ProposalStatus, Topic, Vote, type Ballot } from '@dfinity/nns';
import type { Subscriber } from 'svelte/store';
import type { ProposalsStore } from '../../lib/stores/proposals.store';
import { mockNeuron } from './neurons.mock';

export const deadlineTimestampSeconds = BigInt(Math.round(new Date().getTime() / 1000 + 10000));
...
export const mockProposalsStoreSubscribe = (run: Subscriber<ProposalsStore>): (() => void) => {
	run({ proposals: mockProposals, certified: true });

	return () => undefined;
};

export const mockEmptyProposalsStoreSubscribe = (run: Subscriber<ProposalsStore>): (() => void) => {
