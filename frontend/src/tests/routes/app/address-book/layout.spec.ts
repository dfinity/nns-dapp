...
import Layout from "$routes/(app)/(nns)/address-book/+layout.svelte";
...
describe("Layout", () => {
  const renderAddressBookAndBack = () => {
    page.mock({
      data: {
        universe: OWN_CANISTER_ID_TEXT,
      },
      routeId: AppPath.AddressBook,
    });
    ...
    expect(path).toEqual(AppPath.AddressBook);
