...
import { mockEmptyProposalsStoreSubscribe, mockProposals } from '../mocks/proposals.store.mock';
...
describe('ProposalDetail', () => {
	...
	beforeEach(() => {
		...
		jest
			...
			.mockImplementation(mockRouteStoreSubscribe(`/#/proposal/${mockProposals[0].id}`));
	...
	it('should render loading neurons', async () => {
		...
		await waitFor(() =>
			expect(getByText(en.proposal_detail.loading_neurons, { exact: false })).toBeInTheDocument()
		);
	});
});
