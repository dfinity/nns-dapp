...
describe('Accounts', () => {
	...
	it('should render sns accounts when a project is selected in the dropdown', async () => {
		...
		const selectElement = queryByTestId('select-project-dropdown') as HTMLSelectElement | null;
		...
		await waitFor(() => expect(queryByTestId('sns-accounts-body')).toBeInTheDocument());
	});
	...
	it('should be able to go back to nns after going to a project', async () => {
		...
		await waitFor(() => expect(queryByTestId('accounts-body')).toBeInTheDocument());
	});
	...
	it('should open add account modal', async () => {
		...
		await waitFor(() => {
			...
			expect(getByText(en.accounts.attach_hardware_title, { exact: false })).toBeInTheDocument();
		});
	});
});
