...
import { mockAuthStoreSubscribe, mockPrincipal } from '../mocks/auth.store.mock';
...
describe('Canisters', () => {
	...
	beforeEach(() => {
		authStoreMock = jest.spyOn(authStore, 'subscribe').mockImplementation(mockAuthStoreSubscribe);

		jest.spyOn(canistersStore, 'subscribe').mockImplementation(mockCanistersStoreSubscribe);
	});
	...
	it('should subscribe to store', () => expect(authStoreMock).toHaveBeenCalled());
	...
	it('should render a principal as text', () => {
		...
		expect(getByText(mockPrincipal.toText(), { exact: false })).toBeInTheDocument();
	});
	...
	it('should open the LinkCanisterModal on click to Link Canister', async () => {
		...
		await waitFor(() => expect(queryByTestId('link-canister-modal-title')).toBeInTheDocument());
	});
	...
	it('should open the CreateCanisterModal on click to Create Canister', async () => {
		...
		await waitFor(() => expect(queryByTestId('create-canister-modal-title')).toBeInTheDocument());
	});
});
