...
import { getCanisterDetails, listCanisters } from '../../lib/services/canisters.services';
...
const resetGetCanisterDetailReturn = () => (getCanisterDetailsReturn = defaultReturn);
jest.mock('../../lib/services/canisters.services', () => {
	return {
		listCanisters: jest.fn(),
		routePathCanisterId: () => mockCanister.canister_id.toText(),
		getCanisterDetails: jest.fn().mockImplementation(() => getCanisterDetailsReturn),
...
describe('CanisterDetail', () => {
	jest
		...
		.mockImplementation(
			mockRouteStoreSubscribe(`/#/canister/${mockCanister.canister_id.toText()}`)
		);
	...
	it('should render canister id', async () => {
		...
		await waitFor(() =>
			expect(queryAllByText(mockCanister.canister_id.toText()).length).toBeGreaterThan(0)
		);
	});
	...
	it('should render cards', async () => {
		...
		await waitFor(() => expect(queryByTestId('canister-cycles-card')).toBeInTheDocument());
	...
	it('should not render cards if user is not the controller', async () => {
		...
		await waitFor(() => expect(queryByTestId('canister-details-error-card')).toBeInTheDocument());
		// Waiting for the one above is enough
