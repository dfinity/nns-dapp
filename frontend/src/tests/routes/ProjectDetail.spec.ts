...
import { loadSnsSummary, loadSnsSwapCommitment } from '../../lib/services/sns.services';
...
describe('ProjectDetail', () => {
	describe('present project in store', () => {
		jest
			...
			.mockImplementation(
				mockRouteStoreSubscribe(
					`${AppPath.ProjectDetail}/${mockSnsFullProject.rootCanisterId.toText()}`
				)
			);
		...
		it('should render info section', async () => {
			...
			await waitFor(() => expect(queryByTestId('sns-project-detail-info')).toBeInTheDocument());
		});

		it('should render status section', async () => {
			const { queryByTestId } = render(ProjectDetail);

			await waitFor(() => expect(queryByTestId('sns-project-detail-status')).toBeInTheDocument());
		});
	});
	...
	describe('invalid root canister id', () => {
		jest
			.spyOn(routeStore, 'subscribe')
			.mockImplementation(mockRouteStoreSubscribe(`${AppPath.ProjectDetail}/invalid-project`));
	...
	describe('not found canister id', () => {
		jest
			.spyOn(routeStore, 'subscribe')
			.mockImplementation(mockRouteStoreSubscribe(`${AppPath.ProjectDetail}/aaaaa-aa`));
		jest.spyOn(routeStore, 'replace');
		it('should rediret to launchpad', () => {
			render(ProjectDetail);
