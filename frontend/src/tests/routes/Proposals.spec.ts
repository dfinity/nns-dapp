...
import { GovernanceCanister, type Proposal, type ProposalInfo } from '@dfinity/nns';
...
describe('Proposals', () => {
	const nothingFound = (container: HTMLElement): HTMLParagraphElement | undefined =>
	...
	beforeEach(() => {
		...
		const mockNeuronsStoreSubscribe = (run: Subscriber<NeuronsStore>): (() => void) => {
			run({ neurons: [], certified: true });

			return () => undefined;
		};
		jest.spyOn(neuronsStore, 'subscribe').mockImplementation(mockNeuronsStoreSubscribe);
	});

	describe('Matching results', () => {
		const mockGovernanceCanister: MockGovernanceCanister = new MockGovernanceCanister(
			mockProposals
		);

		const mockLoadProposals = () =>
			jest.spyOn(proposalsStore, 'subscribe').mockImplementation(mockProposalsStoreSubscribe);
		...
		it('should render a spinner while searching proposals', async () => {
			...
			await waitFor(() => expect(getByTestId('proposals-spinner')).not.toBeNull());
		});
		...
		describe('neuron loading', () => {
			...
			it('should render skeletons while loading neurons', async () => {
				...
				await waitFor(() => expect(getByTestId('proposals-loading')).not.toBeNull());
			});
		});
		...
		it('should render proposals', () => {
			...
			expect(getByText((firstProposal.proposal as Proposal).title as string)).toBeInTheDocument();
			expect(getByText((secondProposal.proposal as Proposal).title as string)).toBeInTheDocument();
		});
		...
		it('should hide proposal card if already voted', async () => {
			...
			await waitFor(() =>
				expect(container.querySelectorAll('article').length).toBe(mockProposals.length - 1)
			);
		});
		...
		it('should disable infinite scroll when all proposals loaded', async () => {
			...
			await waitFor(() => expect(getByTestId('proposals-scroll-off')).not.toBeNull());
		});
	...
	describe('No results', () => {
		...
		it('should render not found text', async () => {
			jest.spyOn(proposalsStore, 'subscribe').mockImplementation(mockEmptyProposalsStoreSubscribe);
