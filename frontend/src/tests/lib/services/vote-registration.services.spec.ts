...
import { GovernanceError, Topic, Vote, type NeuronId, type ProposalInfo } from '@dfinity/nns';
...
describe('vote-registration-services', () => {
	...
	describe('success voting', () => {
		beforeAll(() => {
			jest.spyOn(neuronsServices, 'listNeurons').mockImplementation(() => Promise.resolve());
		});
		...
		it('should call the api to register multiple votes', async () => {
			...
			await waitFor(() => expect(spyRegisterVote).toBeCalledTimes(neuronIds.length));
		});
		...
		describe('voting in progress', () => {
			beforeAll(() => {
				...
				jest.spyOn(proposalsApi, 'registerVote').mockImplementation(() => waitForMilliseconds(10));
			});
			...
			it('should update store with a new vote registration', (done) => {
				...
				expect(get(voteRegistrationStore).registrations[0].neuronIds).toEqual(neuronIds);
				expect(get(voteRegistrationStore).registrations[0].proposalInfo.id).toEqual(proposal.id);
				expect(get(voteRegistrationStore).registrations[0].vote).toEqual(Vote.Yes);
			});
			...
			it('should clear the store after registration', async () => {
				...
				await waitFor(() => expect(get(voteRegistrationStore).registrations[0]).not.toBeDefined());
			});
			...
			it('should update successfullyVotedNeuronIds in the store', async () => {
				...
				await waitFor(() => expect(spyOnAddSuccessfullyVotedNeuronId).toBeCalledTimes(3));
						...
						expect.objectContaining({
							content: expect.objectContaining({
								substitutions: {
									...
									$status: replacePlaceholders(en.proposal_detail__vote.vote_status_registering, {
										$completed: `${i}`,
	...
	describe('ignore errors', () => {
		const mockRegisterVoteGovernanceAlreadyVotedError = async (): Promise<void> => {
	...
	describe('register vote errors', () => {
		...
		it('should show error.register_vote on nns-js-based errors', async () => {
			jest.spyOn(proposalsApi, 'registerVote').mockImplementation(mockRegisterVoteError);
		...
		it('should show reason per neuron GovernanceError in detail', async () => {
			jest.spyOn(proposalsApi, 'registerVote').mockImplementation(mockRegisterVoteGovernanceError);
