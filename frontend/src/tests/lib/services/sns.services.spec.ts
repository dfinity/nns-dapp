...
const setNoAccountIdentity = () => (testGetIdentityReturn = Promise.reject(undefined));
const resetAccountIdentity = () => (testGetIdentityReturn = Promise.resolve(mockIdentity));

jest.mock('../../../lib/services/accounts.services', () => {
	return {
		getAccountIdentity: jest.fn().mockImplementation(() => testGetIdentityReturn),
...
describe('sns-services', () => {
	describe('participateInSwap', () => {
		...
		querySnsSwapStates[0].swap[0]!.params[0]!.max_participant_icp_e8s = BigInt(20_000_000_000);
		querySnsSwapStates[0].swap[0]!.params[0]!.min_participant_icp_e8s = BigInt(10_000_000);
		querySnsSwapStates[0].swap[0]!.params[0]!.max_icp_e8s = BigInt(200_000_000_000);
		...
		it('should return success false and not call api if amount lower than minimum', async () => {
			...
			querySnsSwapStates[0].swap[0]!.params[0]!.min_participant_icp_e8s = minimumE8s;
			querySnsSwapStates[0].swap[0]!.params[0]!.max_icp_e8s = BigInt(200_000_000_000);
			const rootCanisterId = Principal.fromText(metadatas[0].rootCanisterId);
			snsQueryStore.setData([metadatas, querySnsSwapStates]);
			const spyParticipate = jest
		...
		it('should return success false and not call api if amount higher than maximum', async () => {
			...
			querySnsSwapStates[0].swap[0]!.params[0]!.max_participant_icp_e8s = maximumE8s;
			querySnsSwapStates[0].swap[0]!.params[0]!.min_participant_icp_e8s = BigInt(10_000_000);
			querySnsSwapStates[0].swap[0]!.params[0]!.max_icp_e8s = BigInt(200_000_000_000);
			const rootCanisterId = Principal.fromText(metadatas[0].rootCanisterId);
			snsQueryStore.setData([metadatas, querySnsSwapStates]);
		...
		it('should return success false and not call api if not enough amount in account balance', async () => {
			...
			const { success } = await participateInSwap({
				amount: TokenAmount.fromE8s({
					amount: account.balance.toE8s() + BigInt(DEFAULT_TRANSACTION_FEE_E8S) + BigInt(10_000),
					token: ICPToken
				}),
				rootCanisterId,
				account
