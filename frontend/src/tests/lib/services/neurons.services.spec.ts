...
const setAccountIdentity = (newIdentity: Identity) => (testIdentity = newIdentity);
...
jest.mock('../../../lib/services/accounts.services', () => {
	return {
		...
		getAccountIdentity: jest.fn().mockImplementation(() => Promise.resolve(testIdentity))
	};
});
...
const resetLedger = () => (getLedgerIdentityImplementation = mockLedgerIdentity);

jest.mock('../../../lib/proxy/ledger.services.proxy', () => {
	return {
		getLedgerIdentityProxy: jest.fn().mockImplementation(() => getLedgerIdentityImplementation())
	};
});
...
describe('neurons-services', () => {
	...
	const spyDisburse = jest.spyOn(api, 'disburse').mockImplementation(() => Promise.resolve());
	...
	const spyAddHotkey = jest.spyOn(api, 'addHotkey').mockImplementation(() => Promise.resolve());
	...
	describe('removeHotkey', () => {
		...
		it('should update neuron and return success when user removes itself', async () => {
			spyGetNeuron.mockImplementation(jest.fn().mockRejectedValue(new NotAuthorizedNeuronError()));
			neuronsStore.pushNeurons({ neurons, certified: true });
	...
	describe('reloadNeuron', () => {
		...
		it('should add neuron to the store', async () => {
			...
			expect(store.neurons?.find(({ neuronId }) => neuronId === mockNeuron.neuronId)).toBeDefined();
		});
	...
	describe('getIdentityOfControllerByNeuronId', () => {
		...
		it('should return identity from authStore first', async () => {
			...
			const identity = await getIdentityOfControllerByNeuronId(controlledNeuron.neuronId);
		...
		it('should return identity from accounts service', async () => {
			const controller = 'gje2w-p7x7x-yuy72-bllam-x2itq-znokr-jnvf6-5dzn4-45jiy-5wvbo-uqe';
			const newIdentity = {
		...
		it('should raise NotAuthorizedNeuronError if contoller is not in authStore nor accounts helper', async () => {
			...
			const neuron = {
				...
				fullNeuron: {
					...mockFullNeuron,
					controller: 'gje2w-p7x7x-yuy72-bllam-x2itq-znokr-jnvf6-5dzn4-45jiy-5wvbo-uqe'
				}
			};
			neuronsStore.setNeurons({ neurons: [neuron], certified: true });
			const call = () => getIdentityOfControllerByNeuronId(neuron.neuronId);
