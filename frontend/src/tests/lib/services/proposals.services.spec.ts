...
import { mockIdentityErrorMsg, resetIdentity, setNoIdentity } from '../../mocks/auth.store.mock';
...
describe('proposals-services', () => {
	...
	describe('load', () => {
		const spyQueryProposal = jest
			...
			.mockImplementation(() => Promise.resolve({ ...mockProposals[0], id: BigInt(666) }));
	...
	describe('empty list', () => {
		afterAll(() => proposalsStore.setProposals({ proposals: [], certified: true }));

		it('should not push empty proposals to the list', async () => {
			jest.spyOn(api, 'queryProposals').mockImplementation(() => Promise.resolve([]));
