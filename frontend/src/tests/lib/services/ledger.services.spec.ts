...
import { mockLedgerIdentifier, MockLedgerIdentity } from '../../mocks/ledger.identity.mock';
...
describe('ledger-services', () => {
		...
		describe('success', () => {
			...
			beforeAll(() =>
				jest
					...
					.mockImplementation(async (): Promise<LedgerIdentity> => mockLedgerIdentity)
			);
	...
	describe('register hardware wallet', () => {
		...
		beforeAll(() => {
			...
			spySyncAccounts = jest.spyOn(accountsServices, 'syncAccounts').mockImplementation(jest.fn());
	...
	describe('get ledger identity', () => {
		...
		it('should return ledger identity', async () => {
			...
			expect(principalToAccountIdentifier(identity.getPrincipal())).toEqual(mockLedgerIdentifier);
		});
	...
	describe('query neurons', () => {
		...
		beforeAll(() => {
			jest.spyOn(api, 'queryNeurons').mockImplementation(() => Promise.resolve(mockNeurons));
		});
