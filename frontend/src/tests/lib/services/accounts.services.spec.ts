...
jest.mock('../../../lib/proxy/ledger.services.proxy', () => {
	return {
		getLedgerIdentityProxy: jest.fn().mockImplementation(() => Promise.resolve(mockIdentity))
	};
});
...
describe('accounts-services', () => {
	describe('services', () => {
		...
		it('should throw errors if transfer params not provided', async () => {
			...
			expect(errDestinationAddress).toEqual('error.transaction_no_destination_address');
	...
	describe('details', () => {
		...
		it('should get account identifier from valid path', () => {
			expect(routePathAccountIdentifier(`/#/wallet/${mockMainAccount.identifier}`)).toEqual({
	...
	describe('getAccountIdentity', () => {
		it('returns user identity if main account', async () => {
			...
			const expectedIdentity = await getAccountIdentity(mockMainAccount.identifier);
		...
		it('returns calls for hardware walleet identity if hardware wallet account', async () => {
			...
			const expectedIdentity = await getAccountIdentity(mockHardwareWalletAccount.identifier);
			expect(expectedIdentity).toBe(mockIdentity);
