...
import { mockAccountDetails, mockSubAccountDetails } from '../../mocks/accounts.store.mock';
...
describe('NNSDapp', () => {
	...
	describe('NNSDapp.addAccount', () => {
		it('returns account identifier when success', async () => {
			const response: string = 'd4685b31b51450508aff0331584df7692a84467b680326f5c5f7d30ae711682f';
	...
	describe('NNSDapp.createSubAccount', () => {
		...
		it('throws error if name too long', async () => {
			...
			const call = async () => nnsDapp.createSubAccount({ subAccountName: 'testSubaccount' });
	...
	describe('NNSDapp.registerHardwareWallet', () => {
		...
		it('should throw register hardware wallet error name too long', async () =>
			await testError(
				...
				new NameTooLongError('error__attach_wallet.create_hardware_wallet_too_long', {
					$accountName: 'test'
				})
			));
