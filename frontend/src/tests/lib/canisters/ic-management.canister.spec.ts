	...
	describe('ICManagementCanister.getCanisterDetails', () => {
		it('returns account identifier when success', async () => {
			const settings = {
				...
				controllers: [
					Principal.fromText('xlmdg-vkosz-ceopx-7wtgu-g3xmd-koiyc-awqaq-7modz-zf6r6-364rh-oqe')
				],
			...
			expect(res).toEqual(toCanisterDetails({ response, canisterId: mockCanisterDetails.id }));
		});
		...
		it('throws UserNotTheControllerError', async () => {
			...
			const call = () => icManagement.getCanisterDetails(Principal.fromText('aaaaa-aa'));
