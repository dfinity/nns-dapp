...
describe('ProjectCommitment', () => {
	...
	it('should render min and max commitment', () => {
		...
		expect(queryByTestId('commitment-max-indicator-value')?.textContent).toEqual(
			`${formatToken({ value: summary.swap.params.max_icp_e8s })} ICP`
		);
		expect(queryByTestId('commitment-min-indicator-value')?.textContent).toEqual(
			`${formatToken({ value: summary.swap.params.min_icp_e8s })} ICP`
		);
	});
	...
	it('should render overall current commitment', () => {
		...
		expect(queryByTestId('sns-project-current-commitment')?.textContent).toEqual(
