...
describe('ProjectInfoSection', () => {
	...
	it('should render title', async () => {
		...
		expect(element.textContent).toEqual(mockSnsFullProject.summary.metadata.name);
	});
	...
	it('should render project link', async () => {
		...
		expect(element).toBeInTheDocument();
		expect(element.getAttribute('href')).toEqual(mockSnsFullProject.summary.metadata.url);
	});
	...
	it('should render project description', async () => {
		...
		expect(element).toBeInTheDocument();
		expect(element.textContent).toEqual(mockSnsFullProject.summary.metadata.description);
	});
	...
	it('should render project logo', async () => {
		...
		expect(element).toBeInTheDocument();
		expect(element.getAttribute('src')).toEqual(mockSnsFullProject.summary.metadata.logo);
	});

	it('should render token name', async () => {
		const { getByTestId } = renderProjectInfoSection(mockSnsFullProject.summary);

		const element = getByTestId('sns-project-detail-info-token-name') as HTMLElement;
		expect(element).toBeInTheDocument();
	...
	it('should render token symbol', async () => {
		...
		const element = getByTestId('sns-project-detail-info-token-symbol') as HTMLElement;
		expect(element).toBeInTheDocument();
		expect(element.textContent).toEqual(mockSnsFullProject.summary.token.symbol);
	});
});
