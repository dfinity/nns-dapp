...
import { mockSwapCommitment, summaryForLifecycle } from '../../../mocks/sns-projects.mock';
...
describe('ProjectUserCommitmentLabel', () => {
	it('should render current commitment label', () => {
		...
		expect(queryByText(en.sns_project_detail.user_current_commitment)).toBeInTheDocument();
	});
	...
	it('should render commitment label (not current)', () => {
		...
		expect(queryByText(en.sns_project_detail.user_commitment)).toBeInTheDocument();
	});
});
