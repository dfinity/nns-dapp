...
describe('ProposalCard', () => {
	const mockGovernanceCanister: MockGovernanceCanister = new MockGovernanceCanister(mockProposals);
	...
	it('should render deadline', () => {
		...
		const durationTillDeadline =
			(mockProposals[0].deadlineTimestampSeconds as bigint) - BigInt(Math.round(Date.now() / 1000));

		const text = `${secondsToDuration(durationTillDeadline)} ${en.proposal_detail.remaining}`;
	...
	it('should render accessible info without label', () => {
		...
		expect(
			container.querySelector(`[aria-label="${en.proposal_detail.type_prefix}"]`)
		).not.toBeNull();
	});
