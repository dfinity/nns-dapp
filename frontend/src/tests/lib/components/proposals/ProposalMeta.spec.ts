...
describe('ProposalMeta', () => {
	...
	it('should not render topic by default', () => {
		...
		expect(() => getByText(new RegExp(`${en.topics[Topic[mockProposalInfo.topic]]}$`))).toThrow();
	});
	...
	it('should render proposer id', () => {
		...
		expect(getByText(new RegExp(`${mockProposalInfo.proposer?.toString()}$`))).toBeInTheDocument();
	});
	...
	it('should render id', () => {
		...
		expect(getByText(new RegExp(`${mockProposalInfo.id?.toString()}$`))).toBeInTheDocument();
	});
