...
describe('SelectProjectDropdown', () => {
	...
	it('should render NNS and project name', () => {
		...
		expect(
			((container.querySelector('option:first-of-type') as HTMLElement).textContent ?? '').trim()
		).toBe(en.core.nns);
		expect(
			((container.querySelector('option:last-of-type') as HTMLElement).textContent ?? '').trim()
	...
	it('should select NNS as default', () => {
		...
		expect(container.querySelector('select')?.value).toBe(OWN_CANISTER_ID.toText());
	});
