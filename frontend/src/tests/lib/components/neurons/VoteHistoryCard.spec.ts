...
describe('VoteHistoryCard', () => {
	...
	it('should render ballots', async () => {
		...
		await waitFor(() =>
			expect(container.querySelectorAll("[data-tid='markdown-text']").length).toEqual(2)
		);

		expect(getByText((mockProposals[0].proposal as Proposal).summary)).toBeInTheDocument();

		expect(getByText((mockProposals[1].proposal as Proposal).summary)).toBeInTheDocument();
	});
	...
	it('should render skeleton texts', async () => {
		...
		expect(container.querySelectorAll('[data-tid="skeleton-text"]').length).toEqual(
			props.neuron.recentBallots.length * 5
		);
	});
});
