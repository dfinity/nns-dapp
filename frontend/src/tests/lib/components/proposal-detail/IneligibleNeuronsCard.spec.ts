...
describe('IneligibleNeuronsCard', () => {
	...
	it('should display texts', () => {
		...
		expect(getByText(en.proposal_detail__ineligible.headline)).toBeInTheDocument();
	...
	it('should display ineligible neurons (created after proposal) ', () => {
		...
		expect((container.querySelector('small') as HTMLElement).textContent).toEqual(
			en.proposal_detail__ineligible.reason_since
		);
	});
