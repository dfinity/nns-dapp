...
describe('ProposalModern', () => {
	...
	it('should be hidden if neurons are not ready', async () => {
		...
		await waitFor(() => expect(queryByTestId('proposal-details-grid')).not.toBeInTheDocument());
	});

	it('should be visible if neurons are ready', async () => {
		const { queryByTestId } = renderProposalModern(true);
		await waitFor(() => expect(queryByTestId('proposal-details-grid')).toBeInTheDocument());
	});

	it('should render system info', async () => {
		const { queryByTestId } = renderProposalModern(true);
		await waitFor(() => expect(queryByTestId('proposal-system-info-details')).toBeInTheDocument());
	});

	it('should render proposer proposal info', async () => {
		const { queryByTestId } = renderProposalModern(true);
		await waitFor(() => expect(queryByTestId('proposal-proposer-info-title')).toBeInTheDocument());
	});
