...
import { mockProposalInfo, mockVoteRegistration } from '../../../../mocks/proposal.mock';
...
describe('VotingConfirmationToolbar', () => {
	...
	it('should disable buttons if nothing is selected', async () => {
		...
		await waitFor(() =>
			expect(container.querySelector('[data-tid="vote-yes"][disabled]')).toBeInTheDocument()
		);
		await waitFor(() =>
			expect(container.querySelector('[data-tid="vote-no"][disabled]')).toBeInTheDocument()
		);
	});

	it('should display Vote.Yes modal', async () => {
		const { container } = render(VotingConfirmationToolbar, { props });
		fireEvent.click(container.querySelector('[data-tid="vote-yes"]') as Element);
		await waitFor(() =>
			expect(container.querySelector('[data-tid="thumb-up"]')).toBeInTheDocument()
		);
	});
	...
	it('should display Vote.No modal', async () => {
		...
		await waitFor(() =>
			expect(container.querySelector('[data-tid="thumb-down"]')).toBeInTheDocument()
		);
	});
	...
	it('should display "total" in modal', async () => {
		...
		fireEvent.click(container.querySelector('[data-tid="vote-yes"]') as Element);
		await waitFor(() =>
			expect(getByText(formatVotingPower(votingPower), { exact: false })).toBeInTheDocument()
		);
	});
	...
	it('should hide confirmation on cancel', async () => {
		...
		fireEvent.click(container.querySelector('[data-tid="vote-yes"]') as Element);
		await waitFor(() =>
			expect(container.querySelector('[data-tid="confirm-no"]')).toBeInTheDocument()
		);
		await fireEvent.click(container.querySelector('[data-tid="confirm-no"]') as Element);
		await waitFor(() =>
			expect(container.querySelector('[data-tid="confirm-no"]')).not.toBeInTheDocument()
		);
	});
	...
	it('should hide confirmation and dispatch on confirm', async () => {
		...
		await fireEvent.click(container.querySelector('[data-tid="vote-no"]') as Element);
		await waitFor(() => container.querySelector('[data-tid="confirm-yes"]'));
		await fireEvent.click(container.querySelector('[data-tid="confirm-yes"]') as Element);
		await waitFor(() =>
			expect(container.querySelector('[data-tid="confirm-no"]')).not.toBeInTheDocument()
		);
	...
	it('should display a question that repeats id and topic', async () => {
		...
		const testLabel = replacePlaceholders(en.proposal_detail__vote.accept_or_reject, {
