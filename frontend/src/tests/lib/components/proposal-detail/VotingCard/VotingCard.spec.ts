...
describe('VotingCard', () => {
	...
	it('should be visible if there are some not-voted-neurons', async () => {
		neuronsStore.setNeurons({ neurons, certified: true });
		const { getByTestId } = renderVotingCard();
		await waitFor(() => expect(getByTestId('voting-confirmation-toolbar')).toBeInTheDocument());
	});
	...
	describe('voting', () => {
		...
		beforeEach(() => {
			...
			jest
				...
				.mockImplementation(
					(): GovernanceCanister => mockGovernanceCanister as unknown as GovernanceCanister
				);
		...
		it('should trigger register-vote and neuron-list updates', async () => {
			...
			await waitFor(() => expect(spyRegisterVote).toBeCalledTimes(neurons.length));
