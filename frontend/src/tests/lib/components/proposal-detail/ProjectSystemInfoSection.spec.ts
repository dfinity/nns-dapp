...
describe('ProposalSystemInfoSection', () => {
	...
	const expectRenderedInfo = async ({
		...
		await waitFor(() => expect(getByTestId(`${testId}-value`)?.textContent).toEqual(value));
		await waitFor(() =>
			expect(getByTestId(`${testId}-description`)?.textContent).toEqual(description)
		);
	};
	...
	const timestampSeconds = (): bigint => BigInt(Math.round(new Date().getTime() / 1000 + 10000));
