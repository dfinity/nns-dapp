...
describe('ProposalNavigation', () => {
	...
	describe('display', () => {
		...
		beforeAll(() => proposalsStore.setProposals({ proposals: mockProposals, certified: true }));

		afterAll(() => proposalsStore.setProposals({ proposals: [], certified: undefined }));
		...
		it('should hide previous', () => {
			...
			expect(getByTestId('proposal-nav-previous')?.classList.contains('hidden')).toBeTruthy();
		});
		...
		it('should display next', () => {
			...
			expect(getByTestId('proposal-nav-next')?.classList.contains('hidden')).not.toBeTruthy();
		});

		it('should hide previous', () => {
			const { getByTestId } = render(ProposalNavigation, { props: propsNext });

			expect(getByTestId('proposal-nav-previous')?.classList.contains('hidden')).not.toBeTruthy();
		});
		...
		it('should display next', () => {
			...
			expect(getByTestId('proposal-nav-next')?.classList.contains('hidden')).toBeTruthy();
		});
	});
