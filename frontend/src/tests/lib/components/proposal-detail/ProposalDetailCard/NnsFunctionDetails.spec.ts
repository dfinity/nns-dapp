...
describe('NnsFunctionDetails', () => {
	...
	it('should start payload loading', async () => {
		const spyOnGetProposalPayload = jest.spyOn(nnsDappMock, 'getProposalPayload');
	...
	it('should display payload fields', async () => {
		...
		await waitFor(() =>
			expect(container.querySelector('.key-value .key')?.innerHTML?.trim()).toEqual('data:')
		);
		expect(container.querySelector('.key-value .value')?.innerHTML).toEqual('"test"');
	});
});
