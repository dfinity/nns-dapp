...
import { mockProposalInfo, proposalActionNnsFunction21 } from '../../../mocks/proposal.mock';
...
describe('ProposalProposerDataSection', () => {
	...
	it('should render entries', async () => {
		...
		await waitFor(() =>
			expect(getByTestId('proposal-proposer-actions-entry-title')).toBeInTheDocument()
		);
		await waitFor(() =>
			expect(getByTestId('proposal-proposer-payload-entry-title')).toBeInTheDocument()
		);
	});
});
