...
describe('DateSeconds', () => {
	...
	const test = ({ container, selector }: { selector: 'p' | 'span'; container: HTMLElement }) => {
		expect(container.querySelector(selector)?.textContent).toContain('January 1, 1970');
		expect(container.querySelector(selector)?.textContent).toContain('12:00 AM');
	};
