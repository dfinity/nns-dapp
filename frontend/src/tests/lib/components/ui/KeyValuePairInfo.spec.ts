...
describe('KeyValuePairInfo', () => {
	...
	it('should toggle to display more information', async () => {
		...
		await waitFor(() =>
			expect(getByTestId('collapsible-content')?.classList.contains('expanded')).toBeTruthy()
		);
		...
		fireEvent.click(button);
		await waitFor(() =>
			expect(getByTestId('collapsible-content')?.classList.contains('expanded')).toBeFalsy()
		);
	});
});
