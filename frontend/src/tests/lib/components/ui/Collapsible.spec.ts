...
describe('Collapsible', () => {
	...
	it('should not render button', () => {
		const { getByText, queryByTestId } = render(CollapsibleTest, props({ expandButton: false }));
		expect(getByText('Jack')).toBeInTheDocument();
		expect(getByText('Sparrow')).toBeInTheDocument();
	...
	it('should render ids', () => {
		...
		expect(container.querySelector('[id="headingCaptain"]')).toBeInTheDocument();
	...
	it('should render and update ARIA attributer', () => {
		...
		expect(container.querySelector('[aria-labelledby="heading_"]')).toBeInTheDocument();
	});
	...
	it('should support initiallyExpanded param', () => {
		const { container } = render(CollapsibleTest, props({ initiallyExpanded: true }));
		expect(container.querySelector('[aria-expanded="true"]')).toBeInTheDocument();
	});
