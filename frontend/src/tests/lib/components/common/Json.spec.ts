...
const testJsonRender = (json: unknown, result?: string) => {
	...
	expect(simplifyJson(container.textContent)).toBe(result ?? simplifyJson(stringifyJson(json)));
};
...
describe('Json', () => {
	...
	it('should render deep structures', () =>
		testJsonRender([
			[
				{
					obj: { test: null },
					list: [0, 1, 2, 'end', { hello: [{ test: ['world', 13, null] }] }, [13]]
				}
			]
		]));
	...
	it('should collaps and expand', async () => {
		...
		expect(simplifyJson(container.textContent)).toBe(simplifyJson(stringifyJson(json)));
	...
	it('should render aria-label for role=button', async () => {
		...
		buttons.forEach((button) => expect(button.getAttribute('aria-label')).toBe(en.core.toggle));
	});
});
