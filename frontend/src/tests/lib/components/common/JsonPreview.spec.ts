...
describe("JsonPreview", () => {
  ...
  it("should render data in tree view", async () => {
    ...
    expect((await po.getTreeText()).trim()).toBe('test "hello world"');
  });
  ...
  it("should expand and collapse in tree view", async () => {
    ...
    expect((await po.getTreeText()).trim()).toBe("data");
    // expand
    await po.clickExpand();
    expect((await po.getTreeText()).trim()).toBe('data test "hello world"');
    // collapse
    await po.clickExpand();
    expect((await po.getTreeText()).trim()).toBe("data");
  });
