...
describe('Proposals', () => {
	...
	it('should display proposal cards', async () => {
		...
		await waitFor(() => expect(queryAllByTestId('proposal-card').length).toBeGreaterThan(0));
	});
	...
	it('should hide skeletons', async () => {
		...
		await waitFor(() =>
			expect(container.querySelectorAll('[data-tid="proposal-card"]').length).toBeGreaterThan(0)
		);
	...
	it('should display no_message', async () => {
		...
		await waitFor(() => expect(queryByText(en.voting.nothing_found)).toBeInTheDocument());
	});
});
