...
import { SnsNeuronPermissionType, SnsSwapLifecycle, type SnsNeuron } from '@dfinity/sns';
...
describe('SnsNeuronCard', () => {
	...
	it('renders the neuron stake and identifier', async () => {
		...
		const stakeText = formatToken({
			value: mockSnsNeuron.cached_neuron_stake_e8s - mockSnsNeuron.neuron_fees_e8s,
			detailed: true
		});
		expect(getByText(stakeText)).toBeInTheDocument();
		expect(queryAllByText(getSnsNeuronIdAsHexString(mockSnsNeuron)).length).toBeGreaterThan(0);
	});
		...
		const { getByText } = render(SnsNeuronCard, {
			props: {
				neuron: {
					...
					dissolve_state: [
						{
							DissolveDelaySeconds: BigInt(nowInSeconds()) + BigInt(MORE_THAN_ONE_YEAR)
						}
					]
				},
				...defaultProps
