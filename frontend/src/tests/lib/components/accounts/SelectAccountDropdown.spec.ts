...
describe('SelectAccountDropdown', () => {
	...
	jest
		...
		.mockImplementation(mockAccountsStoreSubscribe(subaccounts, hardwareWallets));
	...
	it('should select main as default', () => {
		...
		expect(container.querySelector('select')?.value).toBe(mockMainAccount.identifier);
	});
	...
	it('should not render accounts hardware wallets', () => {
		...
		// main + subaccounts
		expect(container.querySelectorAll('option').length).toBe(1 + subaccounts.length);
	});
	...
	it('can select another account', async () => {
		...
		selectElement && expect(selectElement.value).toBe(mockMainAccount.identifier);
		...
		selectElement && expect(selectElement.value).toBe(subaccounts[0].identifier);
	});
});
