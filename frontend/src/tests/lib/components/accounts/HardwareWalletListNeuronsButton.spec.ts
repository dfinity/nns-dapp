...
describe('HardwareWalletListNeuronsButton', () => {
	...
	beforeAll(() => {
		spy = (listNeuronsHardwareWalletProxy as jest.Mock).mockImplementation(async () => ({
			neurons: [mockNeuron]
		}));
	});
	...
	it('should contain an action named list neurons', async () => {
		const { getByText } = renderTestCmp();
		expect(getByText(en.accounts.attach_hardware_show_neurons)).toBeInTheDocument();
	});

	it('should list neurons and open modal', async () => {
		const { getByText, getByTestId } = renderTestCmp();
		await fireEvent.click(getByTestId('ledger-list-button') as HTMLButtonElement);

		await waitFor(() => expect(getByText(en.neurons.title)).toBeInTheDocument());
