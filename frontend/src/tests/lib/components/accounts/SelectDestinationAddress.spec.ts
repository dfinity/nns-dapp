...
describe('SelectDestinationAddress', () => {
	...
	it('should render address input as default', () => {
		...
		expect(container.querySelector("input[name='accounts-address']")).toBeInTheDocument();
	});
	...
	it('should render select account dropdown when toggle is clicked', async () => {
		...
		await waitFor(() => expect(queryByTestId('select-account-dropdown')).toBeInTheDocument());
	});
});
