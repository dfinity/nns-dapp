...
describe('DestinationAddress', () => {
	...
	jest
		...
		.mockImplementation(mockAccountsStoreSubscribe([mockSubAccount, mockSubAccount2]));
	...
	it('should render a list of accounts', () => {
		...
		expect(getByText(mockSubAccount.identifier, { exact: false })).toBeInTheDocument();

		expect(getByText(mockSubAccount2.identifier, { exact: false })).toBeInTheDocument();
	});
