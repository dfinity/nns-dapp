...
describe('HardwareWalletConnectAction', () => {
	...
	it('should render a small explanation text', () => {
		...
		expect(queryByText(en.accounts.connect_hardware_wallet_text)).toBeInTheDocument();
	});
	...
	describe('connected', () => {
		...
		it('should display a connected inforation', async () => {
			...
			await waitFor(() =>
				expect(getByText(en.accounts.hardware_wallet_connected)).toBeInTheDocument()
			);

			expect(getByText(en.core.principal)).toBeInTheDocument();
			expect(getByText(mockIdentity.getPrincipal().toString())).toBeInTheDocument();
		});
	});
