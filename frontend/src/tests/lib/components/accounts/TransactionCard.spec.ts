...
describe('Address', () => {
	...
	it('renders received headline', () => {
		...
		expect(container.querySelector('.title')?.textContent).toBe(en.transaction_names.receive);
	});

	it('renders sent headline', () => {
		const { container } = renderTransactionCard(mockMainAccount, mockSentToSubAccountTransaction);

		expect(container.querySelector('.title')?.textContent).toBe(en.transaction_names.send);
	});
	...
	it('displays transaction date and time', () => {
		...
		expect(container.querySelector('p')?.textContent).toContain('January 1, 1970');
	...
	it('displays identifier for sent', () => {
		const { getByTestId } = renderTransactionCard(mockMainAccount, mockSentToSubAccountTransaction);
		const identifier = getByTestId('identifier')?.textContent;
