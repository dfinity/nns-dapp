...
describe('HardwareWalletNeuronAddHotkeyButton', () => {
	...
	it('should contain a closed modal per default', () => {
		...
		expect(() => getByText(en.accounts.hardware_wallet_add_hotkey_title)).toThrow();
	});
	...
	it('should contain an action named attach neuron', async () => {
		...
		expect(getByText(en.accounts.attach_hardware_neurons_add)).toBeInTheDocument();
	});

	it('should open modal', async () => {
		const { getByText, getByTestId } = render(HardwareWalletAddNeuronHotkeyTest, {
			props
		});
