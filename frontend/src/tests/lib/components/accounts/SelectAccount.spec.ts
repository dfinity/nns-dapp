...
describe('SelectAccount', () => {
	it('should render a skeleton-card until accounts loaded', () => {
		...
		expect(container.querySelector('[data-tid="skeleton-card"]')).toBeInTheDocument();
	});
	...
	it('should not render hardware wallets when prop hideHardwareWalletAccounts is true', () => {
		...
		expect(queryByText(mockHardwareWalletAccount.name as string, { exact: false })).toBeNull();
	...
	it('should render a title with subaccount', async () => {
		...
		await waitFor(() => expect(queryByText(en.accounts.my_accounts)).toBeInTheDocument());
	...
	it('should filter an account for a given identifier', () => {
		...
		expect(() => getByText(mockMainAccount.identifier, { exact: false })).toThrow();
	});
});
