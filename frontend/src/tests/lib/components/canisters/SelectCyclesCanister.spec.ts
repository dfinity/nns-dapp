...
describe('SelectCyclesCanister', () => {
	...
	it('renders button', () => {
		...
		expect(queryByText(en.canisters.review_cycles_purchase)).toBeInTheDocument();
	});
	...
	it('synchronizes icp input to tCycles input', async () => {
		...
		const icpInputElement = container.querySelector<HTMLInputElement>('input[name="icp-amount"]');
		...
		await waitFor(
			() => tCyclesInputElement && expect(tCyclesInputElement.value).toBe(String(value))
		);
	});
	...
	it('synchronizes tCycles input to icp input', async () => {
		...
		await waitFor(() => icpInputElement && expect(icpInputElement.value).toBe(String(value)));
	});

	it('dispatches nnsSelectAmount event on click', async () => {
		const { container, component, queryByTestId } = render(SelectCyclesCanister, { props });
