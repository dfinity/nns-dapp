...
describe('NeuronMetaInfoCard', () => {
	...
	it('renders voting power', () => {
		...
		expect(queryByText(en.neurons.voting_power, { exact: false })).toBeInTheDocument();
		expect(queryByText(formatVotingPower(mockNeuron.votingPower))).toBeInTheDocument();
	});
	...
	it('doest not render voting power if none', () => {
		...
		expect(queryByText(en.neurons.voting_power, { exact: false })).not.toBeInTheDocument();
	});
	...
	it('renders actions', () => {
		...
		expect(queryByText(en.neuron_detail.increase_dissolve_delay)).toBeInTheDocument();
	...
	it('renders no actions if user is not controller', () => {
		...
		expect(queryByText(en.neuron_detail.increase_dissolve_delay)).not.toBeInTheDocument();
		expect(queryByText(en.neuron_detail.start_dissolving)).not.toBeInTheDocument();
		expect(queryByText(en.neuron_detail.increase_stake)).not.toBeInTheDocument();
