...
describe('RemoveCanisterControllerButton', () => {
	...
	it('renders a button', () => {
		...
		expect(queryByTestId('remove-canister-controller-button')).toBeInTheDocument();
	});
	...
	it('opens confirmation modal on click', async () => {
		...
		expect(queryByTestId('remove-canister-controller-confirmation-modal')).toBeInTheDocument();
	});
	...
	it('opens confirmation modal on click and closes it on cancel', async () => {
		...
		await waitFor(() =>
			expect(queryByTestId('remove-canister-controller-confirmation-modal')).not.toBeInTheDocument()
		);
	});
