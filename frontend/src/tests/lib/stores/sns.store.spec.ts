...
describe('sns.store', () => {
	...
	describe('query store', () => {
		...
		it('should update the data', () => {
			...
			expect(
				updatedStore?.metadata.find((summary) => summary.rootCanisterId === rootCanisterId)
			).toEqual(summaries[0]);

			expect(updatedStore?.swaps.find((swap) => swap.rootCanisterId === rootCanisterId)).toEqual(
				swaps[0]
			);
		});
	});
