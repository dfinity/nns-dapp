...
describe('proposals-store', () => {
	...
	describe('filter', () => {
		...
		it('should update topic rewards', () => {
			const filter = [ProposalRewardStatus.AcceptVotes, ProposalRewardStatus.ReadyToSettle];
	...
	describe('votingNeuronSelectStore', () => {
		...
		it('should preserve user selection', () => {
			...
			votingNeuronSelectStore.updateNeurons([...neurons, { ...mockNeuron, neuronId: BigInt(3) }]);
			expect(get(votingNeuronSelectStore).selectedIds).toEqual([0, 2, 3].map(BigInt));
		});
		...
		it('should toggle by neuronId', () => {
			...
			expect(get(votingNeuronSelectStore).selectedIds).toEqual([neuronIds[0], neuronIds[1]]);
		});
	});
