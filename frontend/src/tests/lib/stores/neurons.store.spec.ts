...
describe('neurons-store', () => {
	...
	it('should substitute duplicated neurons', () => {
		...
		const neurons: NeuronInfo[] = [{ ...mockNeuron, neuronId: BigInt(1) }, duplicatedNeuron];
		neuronsStore.setNeurons({ neurons, certified: true });

		const moreNeurons: NeuronInfo[] = [{ ...mockNeuron, neuronId: BigInt(3) }, duplicatedNeuron];
		neuronsStore.pushNeurons({ neurons: moreNeurons, certified: true });
	...
	describe('sortedNeuronStore', () => {
		it('should sort neurons by createdTimestampSeconds', () => {
			...
			expect(get(sortedNeuronStore)).toEqual([neurons[2], neurons[0], neurons[1]]);
		});
	});
});
