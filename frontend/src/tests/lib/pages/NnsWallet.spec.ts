...
describe('NnsWallet', () => {
	...
	const testToolbarButton = ({
		...
		expect((button as HTMLButtonElement).hasAttribute('disabled')).toEqual(disabled);
	};
	...
	describe('no accounts', () => {
		beforeAll(() => {
			jest
				...
				.mockImplementation(mockRouteStoreSubscribe(`/#/wallet/${mockMainAccount.identifier}`));
		});
	...
	describe('accounts loaded', () => {
		beforeAll(() => {
			jest.spyOn(accountsStore, 'subscribe').mockImplementation(mockAccountsStoreSubscribe());
		...
		it('should hide spinner when selected account is loaded', async () => {
			...
			await waitFor(() => expect(container.querySelector('[data-tid="spinner"]')).toBeNull());
		});
		...
		const testModal = async (container: HTMLElement) => {
			const button = container.querySelector('footer div.toolbar button') as HTMLButtonElement;
			await fireEvent.click(button);

			await waitFor(() => expect(container.querySelector('div.modal')).not.toBeNull());
		};
		...
		it('should open transaction modal on step select destination because selected account is current account', async () => {
			...
			await waitFor(() => expect(getByTestId('transaction-step-1')).toBeInTheDocument());
		});
	...
	describe('accounts loaded (Hardware Wallet)', () => {
		beforeAll(() => {
			...
			jest
				.spyOn(routeStore, 'subscribe')
				.mockImplementation(
					mockRouteStoreSubscribe(`/#/wallet/${mockHardwareWalletAccount.identifier}`)
				);
		});
