...
jest.mock('../../../../lib/services/canisters.services', () => {
	return {
		...
		createCanister: jest.fn().mockImplementation(() => Promise.resolve(mockCanister.canister_id))
	};
});
...
describe('CreateCanisterModal', () => {
	jest
		...
		.mockImplementation(mockAccountsStoreSubscribe([mockSubAccount], [mockHardwareWalletAccount]));
	...
	it('should create a canister from ICP and close modal', async () => {
		const { queryByTestId, queryAllByTestId, container, component } = await renderModal({
		...
		await waitFor(() => expect(queryByTestId('select-cycles-screen')).toBeInTheDocument());
		...
		await waitFor(() =>
			expect(queryByTestId('confirm-cycles-canister-screen')).toBeInTheDocument()
		);
	...
	it('should have disabled button when creating canister with less T Cycles than minimum', async () => {
		...
		continueButton && expect(continueButton.hasAttribute('disabled')).toBeTruthy();
	});
});
