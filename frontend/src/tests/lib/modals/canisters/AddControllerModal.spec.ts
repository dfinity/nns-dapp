...
describe('AddControllerModal', () => {
	...
	it('should call addController service and close modal', async () => {
		...
		const { container, queryByTestId, component } = await renderAddControllerModal();
		...
		await waitFor(() => expect(queryByTestId('new-controller-review-screen')).toBeInTheDocument());

		const confirmButton = queryByTestId('confirm-new-canister-controller-button');
