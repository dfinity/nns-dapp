...
import { mockAccountsStoreSubscribe, mockMainAccount } from '../../../mocks/accounts.store.mock';
...
describe('ParticipateSwapModal', () => {
	...
	const renderSwapModal = (swapCommitment: SnsSwapCommitment | undefined = undefined) =>
	...
	const renderEnter10ICPAndNext = async (
		...
		await waitFor(() => expect(participateButton?.hasAttribute('disabled')).toBeFalsy());
	...
	describe('when user has not participated', () => {
		...
		it('should move to the last step, enable button when accepting terms and call participate in swap service', async () => {
			...
			await waitFor(() => expect(confirmButton?.hasAttribute('disabled')).toBeFalsy());
