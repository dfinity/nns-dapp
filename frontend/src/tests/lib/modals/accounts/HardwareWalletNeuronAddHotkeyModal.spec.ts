...
describe('HardwareWalletNeuronAddHotkeyModal', () => {
	...
	beforeAll(() => {
		(getLedgerIdentityProxy as jest.Mock).mockImplementation(() => Promise.resolve(mockIdentity));
	});
	...
	it('should render title', () => {
		...
		expect(getByText(en.accounts.hardware_wallet_add_hotkey_title)).toBeInTheDocument();
	});
	...
	it('should render user principal', () => {
		...
		expect(getByText(mockIdentity.getPrincipal().toString(), { exact: false })).toBeInTheDocument();
	});
	...
	it('should render selected neuron id', () => {
		...
		expect(getByText(mockNeuronStake.neuronId.toString(), { exact: false })).toBeInTheDocument();
	});
	...
	it('should add current user to hotkey and update context', async () => {
		...
		expect(store.neurons.find(({ controlledByNNSDapp }) => !controlledByNNSDapp)).toBeUndefined();
	});
});
