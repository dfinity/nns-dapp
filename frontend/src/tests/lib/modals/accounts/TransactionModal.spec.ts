...
describe('TransactionModal', () => {
	...
	describe('when destination is not provided', () => {
		...
		it('should move to the last step and render review info', async () => {
			...
			expect(
				(getByTestId('transaction-review-source-account').textContent ?? '').includes(
		...
		it('should move to the last step and render passed transaction fee', async () => {
			...
			const { getByText, getByTestId } = await renderEnter10ICPAndNext(undefined, fee);
			...
			expect(getByText(formattedTransactionFeeICP(Number(fee.toE8s())))).toBeInTheDocument();
		});
