...
import { mockAccountsStoreSubscribe, mockSubAccount } from '../../../mocks/accounts.store.mock';
...
describe('NewTransactionModal', () => {
	...
	const goToStep2 = async ({ container, getByText }) => {
		...
		await waitFor(() =>
			expect(getByText(en.accounts.select_destination, { exact: false })).toBeInTheDocument()
		);
	};
	...
	const goToStep3 = async ({ container, getByText }) => {
		...
		await waitFor(() =>
			expect(getByText(en.accounts.enter_icp_amount, { exact: false })).toBeInTheDocument()
		);
	};
	...
	const goToStep4 = async ({
		...
		const button: HTMLButtonElement | null = container.querySelector('button[type="submit"]');
		...
		await waitFor(() =>
			expect(getByText(en.accounts.review_transaction, { exact: false })).toBeInTheDocument()
		);
	};
	...
	it('should navigate back and forth between steps', async () => {
		...
		expect(getByText(en.accounts.select_source, { exact: false })).toBeInTheDocument();
