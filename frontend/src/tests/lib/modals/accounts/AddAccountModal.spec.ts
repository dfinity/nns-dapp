...
jest.mock('../../../../lib/proxy/ledger.services.proxy', () => {
	return {
		connectToHardwareWalletProxy: jest.fn().mockImplementation(async (callback) =>
			callback({
...
describe('AddAccountModal', () => {
	...
	const shouldNavigateSubaccountStep = async ({ queryByText }: RenderResult) => {
		const accountCard = queryByText(en.accounts.new_linked_title);
		expect(accountCard).not.toBeNull();

		accountCard && accountCard.parentElement && (await fireEvent.click(accountCard.parentElement));
	...
	const shouldNavigateHardwareWalletStep = async ({ queryByText }: RenderResult) => {
		const accountCard = queryByText(en.accounts.attach_hardware_title);
		expect(accountCard).not.toBeNull();

		accountCard && accountCard.parentElement && (await fireEvent.click(accountCard.parentElement));

		await waitFor(() => expect(queryByText(en.accounts.attach_hardware_enter_name)).not.toBeNull());
	};
	...
	const testSubaccount = async (extraChecks?: (container: HTMLElement) => void): Promise<void> => {
		const { container, queryByText } = await renderModal({
			component: AddAccountModal
		});
	...
	const goBack = async ({ container, getByText, title }) => {
		...
		await waitFor(() => expect(getByText(title, { exact: false })).toBeInTheDocument());
	};

	const shouldNavigateHardwareWalletConnect = async ({ container, queryByText }: RenderResult) => {
		...
		await waitFor(() =>
			expect(queryByText(en.accounts.connect_hardware_wallet, { exact: false })).not.toBeNull()
		);
	};
	...
	const shouldAttachWallet = async ({ getByTestId, component }: RenderResult) => {
