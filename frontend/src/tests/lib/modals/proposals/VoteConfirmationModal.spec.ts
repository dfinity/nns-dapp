...
describe('VoteConfirmationModal', () => {
	it('should display Adopt state', () => {
		...
		expect(getByText(en.proposal_detail__vote.confirm_adopt_headline)).toBeInTheDocument();
	});
	...
	it('should display Reject state', () => {
		...
		expect(getByText(en.proposal_detail__vote.confirm_reject_headline)).toBeInTheDocument();
	});
	...
	it('should have confirmation buttons', () => {
		...
		expect(container.querySelector('[data-tid="confirm-yes"]')).toBeInTheDocument();
	});
});
