...
describe('FollowNeuronsModal', () => {
	...
	it('renders badge with numbers of followees on the topic', () => {
		...
		const badgeExchange = queryByTestId(`topic-${Topic.ExchangeRate}-followees-badge`);

		expect(badgeExchange).not.toBeNull();
		expect(badgeExchange?.innerHTML).toBe('2');

		const badgeGovernance = queryByTestId(`topic-${Topic.Governance}-followees-badge`);
	...
	it('displays the followees of the user in specific topic', async () => {
		...
		const topicSection = queryByTestId(`follow-topic-${Topic.ExchangeRate}-section`);
		expect(topicSection).not.toBeNull();

		if (topicSection !== null) {
			const followeeElements = topicSection?.querySelectorAll('[data-tid="current-followee-item"]');
