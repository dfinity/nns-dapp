...
describe('SplitNeuronModal', () => {
	const renderSplitNeuronModal = async (neuron: NeuronInfo): Promise<RenderResult> => {
		return renderModal({
	...
	it('should have disabled button if value is 0', async () => {
		...
		inputElement && (await fireEvent.input(inputElement, { target: { value: 0 } }));
	...
	it('should call split neuron service if amount is valid', async () => {
		...
		inputElement && (await fireEvent.input(inputElement, { target: { value: 2.2 } }));
