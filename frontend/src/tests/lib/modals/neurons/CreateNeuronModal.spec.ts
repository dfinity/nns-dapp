...
import { fireEvent, waitFor, type BoundFunction, type queries } from '@testing-library/svelte';
...
jest.mock('../../../../lib/services/neurons.services', () => {
	return {
		stakeNeuron: jest.fn().mockImplementation(() => Promise.resolve(newNeuron.neuronId)),
...
describe('CreateNeuronModal', () => {
	describe('main account selection', () => {
		beforeEach(() => {
			...
			jest.spyOn(LedgerCanister, 'create').mockImplementation(() => mock<LedgerCanister>());
			jest.spyOn(GovernanceCanister, 'create').mockImplementation(() => mock<GovernanceCanister>());
		});
		...
		it('should move to update dissolve delay after creating a neuron', async () => {
			...
			await waitFor(() =>
				expect(container.querySelector("[data-tid='go-confirm-delay-button']")).not.toBeNull()
			);
		});
		...
		it('should have the update delay button disabled', async () => {
			...
			const updateDelayButton = container.querySelector('[data-tid="go-confirm-delay-button"]');
		...
		it('should have disabled button for dissolve less than six months', async () => {
			...
			inputRange && (await fireEvent.input(inputRange, { target: { value: FIVE_MONTHS } }));
		...
		it('should be able to create a neuron and see the stake of the new neuron in the dissolve modal', async () => {
			...
			expect(getByText(formatVotingPower(neuronStakeE8s), { exact: false })).not.toBeNull();
		});
		...
		it('should be able to change dissolve delay in the confirmation screen', async () => {
			...
			inputRange && (await fireEvent.input(inputRange, { target: { value: ONE_YEAR } }));

			const goToConfirmDelayButton = container.querySelector(
				'[data-tid="go-confirm-delay-button"]'
			);
			await waitFor(() => expect(goToConfirmDelayButton?.getAttribute('disabled')).toBeNull());
			...
			const confirmButton = container.querySelector('[data-tid="confirm-delay-button"]');
		...
		it('should go to edit followers when skipping dissolve delay', async () => {
			...
			await waitFor(() => expect(queryByTestId('edit-followers-screen')).not.toBeNull());
		});
	});
	...
	describe('hardware wallet account selection', () => {
		beforeEach(() => {
			jest
				.spyOn(accountsStore, 'subscribe')
				.mockImplementation(mockAccountsStoreSubscribe([], [mockHardwareWalletAccount]));
			jest.spyOn(authStore, 'subscribe').mockImplementation(mockAuthStoreSubscribe);
		});
		...
		const createNeuron = async ({
			...
			const hardwareWalletAccount = queryByText(mockHardwareWalletAccount.name as string, {
				exact: false
			});
		...
		it('should create neuron for hardwareWallet and close modal if hotkey is not added', async () => {
			const { container, queryByTestId, queryByText, component } = await renderModal({
				component: CreateNeuronModal
			});
			...
			await waitFor(() => expect(queryByTestId('add-principal-to-hotkeys-modal')).not.toBeNull());
		...
		it('should create neuron for hardwareWallet and add dissolve delay', async () => {
			...
			const addHotkeyButton = queryByTestId('confirm-add-principal-to-hotkey-modal');
