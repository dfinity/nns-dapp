...
describe('DisburseNeuronModal', () => {
	const renderDisburseModal = async (neuron: NeuronInfo): Promise<RenderResult> => {
		return renderModal({
	...
	const spyNavigate = jest.spyOn(routeStore, 'replace').mockImplementation(jest.fn());
	...
	it('should be able to select an account', async () => {
		const { queryAllByTestId, queryByTestId } = await renderDisburseModal(mockNeuron);
	...
	it('should be able to add address in input', async () => {
		...
		addressInput && (await fireEvent.input(addressInput, { target: { value: address } }));
