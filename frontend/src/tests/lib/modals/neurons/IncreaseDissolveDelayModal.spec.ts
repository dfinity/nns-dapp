...
describe('IncreaseDissolveDelayModal', () => {
	const renderIncreaseDelayModal = async (neuron: NeuronInfo): Promise<RenderResult> => {
		return renderModal({
	...
	it('should be able to change dissolve delay in the confirmation screen', async () => {
		...
		const goToConfirmDelayButton = container.querySelector('[data-tid="go-confirm-delay-button"]');
	...
	it('should not be able to change dissolve delay below current value', async () => {
		...
		await waitFor(() => expect(container.querySelector('input[type="range"]')).not.toBeNull());
		const inputRange = container.querySelector<HTMLInputElement>('input[type="range"]');
		expect(inputRange).not.toBeNull();
		...
		const goToConfirmDelayButton = container.querySelector('[data-tid="go-confirm-delay-button"]');
		await waitFor(() => expect(goToConfirmDelayButton?.getAttribute('disabled')).not.toBeNull());
		inputRange && (await waitFor(() => expect(inputRange.value).toBe(String(SECONDS_IN_YEAR))));
	});
});
