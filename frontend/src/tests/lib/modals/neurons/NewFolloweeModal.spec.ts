...
import { mockFullNeuron, mockKnownNeuron, mockNeuron } from '../../../mocks/neurons.mock';
...
describe('NewFolloweeModal', () => {
	...
	it('renders an input for a neuron address', () => {
		...
		const inputElement = container.querySelector('[name="new-followee-address"]');
	...
	it('adds a followee from a valid address', async () => {
		...
		expect(inputElement).toBeInTheDocument();
		inputElement && (await fireEvent.input(inputElement, { target: { value: 123 } }));
	...
	it('renders known neurons to unfollow', async () => {
		...
		const knownNeuronElement = queryByTestId(`known-neuron-item-${mockKnownNeuron.id}`);
		...
		knownNeuronButton && expect(knownNeuronButton.innerHTML).toEqual(en.new_followee.unfollow);
	});
