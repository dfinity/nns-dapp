...
describe('MergeMaturityModal', () => {
	...
	it('should call mergeMaturity service on confirm click', async () => {
		...
		rangeElement && (await fireEvent.input(rangeElement, { target: { value: 50 } }));

		const selectMaturityButton = queryByTestId('select-maturity-percentage-button');
		expect(selectMaturityButton).toBeInTheDocument();
		selectMaturityButton && (await fireEvent.click(selectMaturityButton));

		await waitFor(() => expect(queryByTestId('confirm-action-screen')).toBeInTheDocument());
