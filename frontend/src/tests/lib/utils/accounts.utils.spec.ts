...
describe('accounts-utils', () => {
	...
	describe('getPrincipalFromString', () => {
		...
		it('returns principal when valid address', () => {
			...
			expect(
				getPrincipalFromString('djzvl-qx6kb-xyrob-rl5ki-elr7y-ywu43-l54d7-ukgzw-qadse-j6oml-5qe')
	...
	describe('getAccountFromStore', () => {
		...
		it('should not return an account if no identifier is provided', () => {
			expect(getAccountFromStore({ identifier: undefined, accountsStore })).toBeUndefined();
		});

		it('should find no account if not matches', () => {
			expect(getAccountFromStore({ identifier: 'aaa', accountsStore })).toBeUndefined();
		});
