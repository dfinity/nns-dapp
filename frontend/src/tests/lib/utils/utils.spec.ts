...
describe('utils', () => {
	...
	describe('stringifyJson', () => {
		...
		it('should stringify JSON with bigint', () => {
			expect(stringifyJson({ a: BigInt(123) })).toBe(JSON.stringify({ a: '123' }));
		});

		it('should support the indentation', () => {
			expect(stringifyJson(SAMPLE, { indentation: 2 })).toBe(JSON.stringify(SAMPLE, null, 2));
		});

		it('should convert bigints to function call in devMode', () => {
			expect(
				stringifyJson({ value: BigInt('123456789012345678901234567890') }, { devMode: true })
		...
		it('should not call toString() for Principal alike objects', () => {
			expect(stringifyJson({ _isPrincipal: true })).toBe(`{"_isPrincipal":true}`);
		});
	});
	...
	describe('bytesToHexString and hexStringToBytes', () => {
		it('converts bytes to string and back', () => {
			...
			expect(hexStringToBytes(bytesToHexString([1, 255, 3, 0]))).toEqual([1, 255, 3, 0]);
		});
	});
	...
	describe('removeKeys', () => {
		it('removes the keys passed', () => {
			...
			expect(Object.keys(removeKeys({ obj, keysToRemove: ['b', 'a', 'c'] })).length).toBe(0);
		});
