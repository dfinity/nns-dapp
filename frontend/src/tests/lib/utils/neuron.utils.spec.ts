import { ICPToken, NeuronState, TokenAmount, Topic, Vote, type BallotInfo } from '@dfinity/nns';
...
import { DEFAULT_TRANSACTION_FEE_E8S, E8S_PER_ICP } from '../../../lib/constants/icp.constants';
import { MAX_NEURONS_MERGED, MIN_NEURON_STAKE } from '../../../lib/constants/neurons.constants';
...
import { mockHardwareWalletAccount, mockMainAccount } from '../../mocks/accounts.store.mock';
...
describe('neuron-utils', () => {
	...
	describe('votingPower', () => {
		...
		it('should return zero for delays less than six months', () => {
			expect(votingPower({ stake: BigInt(2), dissolveDelayInSeconds: 100 })).toBe(BigInt(0));
		});
	...
	describe('sortNeuronsByCreatedTimestamp', () => {
		it('should sort neurons by createdTimestampSeconds', () => {
			...
			expect(sortNeuronsByCreatedTimestamp([neuron3, neuron2, neuron1])).toEqual([
				neuron3,
				neuron2,
				neuron1
			]);
			expect(sortNeuronsByCreatedTimestamp([neuron2, neuron1, neuron3])).toEqual([
				neuron3,
				neuron2,
				neuron1
			]);
	...
	describe('isNeuronControllable', () => {
		it('should return true if neuron controller is the current main account', () => {
			...
			expect(isNeuronControllable({ neuron, identity: mockIdentity, accounts })).toBe(true);
		});
		...
		it('should return false if fullNeuron not defined', () => {
			...
			expect(isNeuronControllable({ neuron, identity: mockIdentity, accounts })).toBe(false);
		});
	...
	describe('isNeuronControllableByUser', () => {
		it('should return true if neuron controller is the current identity principal', () => {
			...
			expect(isNeuronControllableByUser({ neuron, mainAccount: mockMainAccount })).toBe(true);
		});
		...
		it('should return false if fullNeuron not defined', () => {
			...
			expect(isNeuronControllableByUser({ neuron, mainAccount: mockMainAccount })).toBe(false);
		});
	...
	describe('isNeuronControlledByHardwareWallet', () => {
		it('should return false if neuron controller is the current main account', () => {
			...
			expect(isNeuronControlledByHardwareWallet({ neuron, accounts })).toBe(false);
		});
		...
		it('should return true if neuron controller is hardware wallet', () => {
			...
			expect(isNeuronControlledByHardwareWallet({ neuron, accounts })).toBe(true);
		});
	});
	...
	describe('isEnoughMaturityToSpawn', () => {
		it('return false if just enough ICP to create a neuron without taking into account variance', () => {
			...
			expect(isEnoughMaturityToSpawn({ neuron: neuron2, percentage: 50 })).toBe(false);
		});
		it('return true if enough ICP to spawn a neuron', () => {
			...
			expect(isEnoughMaturityToSpawn({ neuron: neuron2, percentage: 50 })).toBe(true);
		});
		it('returns false if not enough ICP to spawn a neuron', () => {
			...
			expect(isEnoughMaturityToSpawn({ neuron: neuron2, percentage: 10 })).toBe(false);
		});
	});
	...
	describe('isIdentityController', () => {
		it('return true if identity is the controller of the neuron', () => {
			...
			expect(isIdentityController({ neuron, identity: mockIdentity })).toBe(true);
		});
		...
		it('return false if identity is not the controller of the neuron', () => {
			...
			expect(isIdentityController({ neuron, identity: mockIdentity })).toBe(false);
		});
		...
		it('return true if identity is null', () => {
			expect(isIdentityController({ neuron: mockNeuron, identity: null })).toBe(false);
		});
	});
	...
	describe('mapMergeableNeurons', () => {
		...
		it(`does not allow to have more mergeable once ${MAX_NEURONS_MERGED} is reached`, () => {
			...
			const wrappedNeurons = mapMergeableNeurons({
				neurons: [neuron, neuronFollowingManageNeuron, neuron3, neuron4, neuron5],
				accounts: {
					main: mockMainAccount
				},
	...
	describe('followeesByTopic', () => {
		...
		it('should return undefined if topic not found', () => {
			expect(followeesByTopic({ neuron, topic: Topic.ManageNeuron })).toBeUndefined();
		});

		it('should return undefined if no neuron', () => {
			expect(followeesByTopic({ neuron: undefined, topic: Topic.ManageNeuron })).toBeUndefined();
		});
	...
	describe('votedNeuronDetails', () => {
		it('should return array of CompactNeuronInfo', () => {
			...
			compactNeuron1 && expect(compactNeuron1.votingPower).toBe(ballot1.votingPower);
		});
	...
	describe('hasEnoughMaturityToMerge', () => {
		...
		it('returns true if maturity larger than needed', () => {
			const neuron = {
				...
				fullNeuron: {
					...mockFullNeuron,
					maturityE8sEquivalent: BigInt(DEFAULT_TRANSACTION_FEE_E8S) + BigInt(1000)
				}
			};
		...
		it('returns false if maturity smaller than needed', () => {
			const neuron = {
				...
				fullNeuron: {
					...mockFullNeuron,
					maturityE8sEquivalent: BigInt(DEFAULT_TRANSACTION_FEE_E8S) - BigInt(100)
				}
			};
			expect(hasEnoughMaturityToMerge({ neuron, fee: 10_000 })).toBe(false);
		});
