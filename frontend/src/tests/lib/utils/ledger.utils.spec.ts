...
describe('ledger-utils', () => {
	describe('decodePublicKey', () => {
		...
		it('should throw an error because public key cannot be fetched', () => {
			const call = () =>
				decodePublicKey({
					principalText,
					publicKey: fromHexString(rawPublicKeyHex) as unknown as Buffer,
					returnCode: ExtendedLedgerError.CannotFetchPublicKey as unknown as LedgerError
				} as ResponseAddress);

			expect(call).toThrow(new LedgerErrorKey('error__ledger.fetch_public_key'));
		});
		...
		it('should throw an error because principal does not match', () => {
			...
			expect(call).toThrow(new LedgerErrorKey('error__ledger.principal_not_match'));
		});
	...
	describe('decodeSignature', () => {
		...
		it('should throw an error if transaction is rejected', () => {
			...
			expect(call).toThrow(new LedgerErrorKey('error__ledger.user_rejected_transaction'));
		});
		...
		it('should throw an error if signature too short', () => {
			...
			expect(call).toThrow(
				new LedgerErrorKey(`Signature must be 64 bytes long (is ${test.length})`)
			);
		});
