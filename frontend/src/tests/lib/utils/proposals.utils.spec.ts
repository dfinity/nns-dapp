import type { Ballot, ExecuteNnsFunction, NeuronInfo, Proposal, ProposalInfo } from '@dfinity/nns';
...
describe('proposals-utils', () => {
	it('should find no last proposal id', () => expect(lastProposalId([])).toBeUndefined());

	it('should find fist action key', () =>
		expect(proposalFirstActionKey(proposalWithNnsFunctionAction)).toEqual('ExecuteNnsFunction'));
	...
	describe('proposalActionFields', () => {
		it('should filter action fields', () => {
			...
			expect(fields.map(([key]) => key).join()).toEqual('nodeProvider,amountE8s,rewardMode');
		});
		...
		it('should select new neurons', () => {
			expect(
				preserveNeuronSelectionAfterUpdate({
					selectedIds: [].map(BigInt),
					neurons: [neuron(0), neuron(1), neuron(2)],
					updatedNeurons: [neuron(0), neuron(1), neuron(2), neuron(3), neuron(4)]
				})
	...
	describe('proposalIdSet', () => {
		it('should return a set with ids', () => {
			...
			expect(Array.from(idSet).sort()).toStrictEqual(proposals.map(({ id }) => id).sort());
		});
	...
	describe('mapProposalInfo', () => {
		...
		it('should map proposalInfo fields', () => {
			...
			expect(deadline).toEqual(deadlineTimestampSeconds - BigInt(nowInSeconds()));
			...
			expect(statusString).toEqual(en.status[ProposalStatus[proposalInfo.status]]);
			expect(statusDescription).toEqual(en.status_description[ProposalStatus[proposalInfo.status]]);
		...
		it('should map action to type', () => {
			...
			expect(en.actions_description.RegisterKnownNeuron).toEqual(typeDescription);
		});
		...
		it('should map nns function to type', () => {
			...
			expect(en.nns_functions_description.NnsCanisterInstall).toEqual(typeDescription);
		});
	});
	...
	describe('proposalsHaveSameIds', () => {
		...
		it('should comprare', () => {
			expect(proposalsHaveSameIds({ proposalsA: [], proposalsB: [] })).toBeTruthy();
			expect(
				proposalsHaveSameIds({
		...
		it('should be open for votes quiet threshold', () => {
			...
			expect(
				isProposalDeadlineInTheFuture({
					...
					deadlineTimestampSeconds: undefined,
					topic: Topic.Governance,
					proposalTimestampSeconds: BigInt(Math.round(nowSeconds - 3600 * 24 * 3))
				})
			).toBeTruthy();
		});
		...
		it('should not be open for votes quiet threshold', () => {
			...
			expect(
				isProposalDeadlineInTheFuture({
					...
					deadlineTimestampSeconds: undefined,
					topic: Topic.Governance,
					proposalTimestampSeconds: BigInt(Math.round(nowSeconds - 3600 * 24 * 5))
				})
			).toBeFalsy();
		});
	});
