...
describe('markdown.utils', () => {
	describe('targetBlankLinkRenderer', () => {
		...
		it('should skip href if not provided', () => {
			expect(targetBlankLinkRenderer(null, 'title', 'text')).toEqual(`<a title="title">text</a>`);
		...
		it('should render href of title if no text', () => {
			...
			expect(targetBlankLinkRenderer(null, 'title', '')).toEqual(`<a title="title">title</a>`);
		});
	});
	...
	describe('sanitize and markdown', () => {
		...
		it('should sanitize HTML', () => {
			...
			expect(sanitize('<math><mi//xlink:href="data:x,<script>alert(4)</script>">')).toEqual(
				'<math><mi></mi></math>'
			);
