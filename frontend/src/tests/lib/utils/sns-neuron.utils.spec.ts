...
describe('sns-neuron utils', () => {
	describe('sortNeuronsByCreatedTimestamp', () => {
		it('should sort neurons by created_timestamp_seconds', () => {
			...
			expect(sortSnsNeuronsByCreatedTimestamp([neuron3, neuron2, neuron1])).toEqual([
				neuron3,
				neuron2,
				neuron1
			]);
			expect(sortSnsNeuronsByCreatedTimestamp([neuron2, neuron1, neuron3])).toEqual([
				neuron3,
				neuron2,
				neuron1
			]);
	...
	describe('getSnsDissolvingTimeInSeconds', () => {
		...
		it('returns time in seconds until dissolve', () => {
			...
			expect(getSnsDissolvingTimeInSeconds(neuron)).toBe(BigInt(SECONDS_IN_YEAR));
		});
	});
	...
	describe('getSnsNeuronIdAsHexString', () => {
		it('returns id numbers concatenated', () => {
			const id = [
				154, 174, 251, 49, 236, 17, 214, 189, 195, 140, 58, 89, 61, 29, 138, 113, 79, 48, 136, 37,
				96, 61, 215, 50, 182, 65, 198, 97, 8, 19, 238, 36
			];
	...
	describe('routePathSnsNeuronRootCanisterId', () => {
		...
		it('should not get root canister id from invalid path', async () => {
			...
			expect(routePathSnsNeuronRootCanisterId('/#/u/124/neuron')).toBeUndefined();
	...
	describe('canIdentityManageHotkeys', () => {
		...
		it('returns true when user has voting rights', () => {
			const controlledNeuron: SnsNeuron = {
				...mockSnsNeuron,
				permissions: [...hotkeys, mockIdentity.getPrincipal().toText()].map(addVotePermission)
			};
			expect(
				canIdentityManageHotkeys({
	...
	describe('getSnsNeuronHotkeys', () => {
		...
		it('returns array of principal ids', () => {
			const controlledNeuron: SnsNeuron = {
				...mockSnsNeuron,
				permissions: hotkeys.map(addVotePermission).concat(controllerPermission)
			};
		...
		it("doesn't return the controller", () => {
			...
			expect(expectedHotkeys.includes(mockIdentity.getPrincipal().toText())).toBe(false);
		});
	});
