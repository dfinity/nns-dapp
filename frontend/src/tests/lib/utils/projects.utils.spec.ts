...
describe('project-utils', () => {
	...
	describe('can user participate to swap', () => {
		it('cannot participate to swap if no summary or swap information', () => {
			...
			expect(canUserParticipateToSwap({ summary: null, swapCommitment: undefined })).toBeFalsy();
			expect(canUserParticipateToSwap({ summary: undefined, swapCommitment: null })).toBeFalsy();
			expect(canUserParticipateToSwap({ summary: null, swapCommitment: null })).toBeFalsy();
		});
			...
			expect(
				canUserParticipateToSwap({
					...
					swapCommitment: {
						...
						myCommitment: {
							icp: [createTransferableAmount(mockSnsParams.max_participant_icp_e8s)]
						}
					}
				})
			).toBeFalsy();
	...
	describe('currentUserMaxCommitment', () => {
		...
		it('returns the remainder to the user maximum if already participated', () => {
			...
			expect(currentUserMaxCommitment(validProject)).toEqual(userMax - userCommitment);
		});
		...
		it('returns the remainder to the project maximum if remainder lower than user max', () => {
			...
			expect(currentUserMaxCommitment(validProject)).toEqual(projectMax - projectCommitment);
		});
	...
	describe('projectRemainingAmount', () => {
		it('returns remaining amount taking into account current commitment', () => {
			...
			expect(projectRemainingAmount(summary)).toEqual(projectMax - projectCommitment);
		});
	});
