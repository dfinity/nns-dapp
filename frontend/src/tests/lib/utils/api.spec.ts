...
describe('api-utils', () => {
	describe('queryAndUpdate', () => {
		it('should request twice', async () => {
			const request = jest.fn().mockImplementation(() => Promise.resolve({ certified: true }));
		...
		it('should support "query_and_update" strategy', async () => {
			...
			const request = jest.fn().mockImplementation(({ certified }: { certified: boolean }) => {
		...
		it('should catch errors', async () => {
			const request = jest.fn().mockImplementation(() => Promise.reject('test'));
			const onLoad = jest.fn();
			const onError = jest.fn();
