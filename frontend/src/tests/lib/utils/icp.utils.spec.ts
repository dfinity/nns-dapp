...
describe('icp-utils', () => {
	it('should format icp', () => {
		...
		expect(formatToken({ value: BigInt(200000000000000) })).toEqual(`2'000'000.00`);
	});

	it('should format icp detailed', () => {
		expect(formatToken({ value: BigInt(0), detailed: true })).toEqual('0');
		expect(formatToken({ value: BigInt(100), detailed: true })).toEqual('0.000001');
		expect(formatToken({ value: BigInt(100000000), detailed: true })).toEqual('1.00');
		expect(formatToken({ value: BigInt(1000000000), detailed: true })).toEqual('10.00');
		expect(formatToken({ value: BigInt(1010000000), detailed: true })).toEqual('10.10');
		expect(formatToken({ value: BigInt(1012300000), detailed: true })).toEqual('10.123');
		expect(formatToken({ value: BigInt(20000000000), detailed: true })).toEqual('200.00');
		expect(formatToken({ value: BigInt(20000000001), detailed: true })).toEqual('200.00000001');
		expect(formatToken({ value: BigInt(200000000000), detailed: true })).toEqual(`2'000.00`);
		expect(formatToken({ value: BigInt(200000000000000), detailed: true })).toEqual(`2'000'000.00`);
	});
	...
	it('should format a specific transaction fee', () =>
		expect(formattedTransactionFeeICP(DEFAULT_TRANSACTION_FEE_E8S)).toEqual('0.0001'));
	...
	describe('convertIcpToTCycles', () => {
		it('converts ICP to TCycles', () => {
			expect(convertIcpToTCycles({ icpNumber: 1, exchangeRate: BigInt(10_000) })).toBe(1);
			expect(convertIcpToTCycles({ icpNumber: 2.5, exchangeRate: BigInt(10_000) })).toBe(2.5);
			expect(convertIcpToTCycles({ icpNumber: 2.5, exchangeRate: BigInt(20_000) })).toBe(5);
			expect(convertIcpToTCycles({ icpNumber: 1, exchangeRate: BigInt(15_000) })).toBe(1.5);
		});
	});

	describe('convertTCyclesToIcpNumber', () => {
		it('converts TCycles to number', () => {
			expect(convertTCyclesToIcpNumber({ tCycles: 1, exchangeRate: BigInt(10_000) })).toBe(1);
			expect(
				convertTCyclesToIcpNumber({
					tCycles: 2.5,
			...
			expect(convertTCyclesToIcpNumber({ tCycles: 1, exchangeRate: BigInt(15_000) })).toBe(2 / 3);
			expect(
				convertTCyclesToIcpNumber({
					tCycles: 4.32,
					exchangeRate: BigInt(10_000)
