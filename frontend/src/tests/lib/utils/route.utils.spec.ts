...
describe('route-utils', () => {
	...
	const resetTestURL = (baseUrl) => window.history.replaceState({}, 'Test Title', baseUrl);
	...
	describe('base href is /', () => {
		...
		beforeAll(() => jest.spyOn(routeUtils, 'baseHref').mockImplementation(() => '/'));

		afterEach(() => resetTestURL(baseUrl));

		it('should return the pathname and the hash without base href', () => testRoutePath());
