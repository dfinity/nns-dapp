...
describe('transactions-utils', () => {
	...
	describe('mapToSelfTransaction', () => {
		it('should map to self transactions', () => {
			...
			const toSelfTransactions = transactions.map(({ toSelfTransaction }) => toSelfTransaction);

			expect(toSelfTransactions).toEqual([false, true, false, false, false, true, false]);
		});
	});

	describe('mapTransaction', () => {
		it('should map Send transaction', () => {
			const { type, isReceive, isSend, from, to, displayAmount, date } = mapTransaction({
				transaction: mockSentToSubAccountTransaction,
				account: mockMainAccount
			});
			// eslint-disable-next-line @typescript-eslint/no-explicit-any
			const amount = (mockSentToSubAccountTransaction.transfer as any)?.Send?.amount.e8s as bigint;
			// eslint-disable-next-line @typescript-eslint/no-explicit-any
			const fee = (mockSentToSubAccountTransaction.transfer as any)?.Send?.fee.e8s as bigint;
			...
			expect(+date).toBe(
				Number(mockSentToSubAccountTransaction.timestamp.timestamp_nanos / BigInt(1e6))
			);
		});
		...
		it('should map Receive transaction', () => {
			...
			// eslint-disable-next-line @typescript-eslint/no-explicit-any
			const amount = (mockReceivedFromMainAccountTransaction.transfer as any)?.Receive?.amount
				.e8s as bigint;
			// eslint-disable-next-line @typescript-eslint/no-explicit-any
			const fee = (mockReceivedFromMainAccountTransaction.transfer as any)?.Receive?.fee
				.e8s as bigint;

			expect(type).toBe(transactionType(mockReceivedFromMainAccountTransaction));
			...
			expect(+date).toBe(
				Number(mockReceivedFromMainAccountTransaction.timestamp.timestamp_nanos / BigInt(1e6))
			);
		});
