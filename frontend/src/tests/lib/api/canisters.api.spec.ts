...
import { AccountIdentifier, ICPToken, LedgerCanister, SubAccount, TokenAmount } from '@dfinity/nns';
...
import { CREATE_CANISTER_MEMO, TOP_UP_CANISTER_MEMO } from '../../../lib/constants/api.constants';
...
import { mockCanisterDetails, mockCanisterSettings } from '../../mocks/canisters.mock';
...
describe('canisters-api', () => {
	...
	beforeEach(() => {
		...
		jest.spyOn(ICManagementCanister, 'create').mockImplementation(() => mockICManagementCanister);

		jest.spyOn(LedgerCanister, 'create').mockImplementation(() => mockLedgerCanister);
	});
	...
	describe('updateSettings', () => {
		...
		it('should call the ic management canister to update settings with partial settings', async () => {
			const settings = {
				controllers: ['xlmdg-vkosz-ceopx-7wtgu-g3xmd-koiyc-awqaq-7modz-zf6r6-364rh-oqe']
			};
			mockICManagementCanister.updateSettings.mockResolvedValue(undefined);
			await updateSettings({
				identity: mockIdentity,
	...
	describe('queryCanisterDetails', () => {
		it('should call IC Management Canister with canister id', async () => {
			mockICManagementCanister.getCanisterDetails.mockResolvedValue(mockCanisterDetails);

			const response = await queryCanisterDetails({
				identity: mockIdentity,
				canisterId: mockCanisterDetails.id
			});
			expect(mockICManagementCanister.getCanisterDetails).toBeCalledWith(mockCanisterDetails.id);
	...
	describe('getIcpToCyclesExchangeRate', () => {
		it('should call CMC to get conversion rate', async () => {
			mockCMCCanister.getIcpToCyclesConversionRate.mockResolvedValue(BigInt(10_000));
	...
	describe('createCanister', () => {
		...
		it('should make a transfer, notify and attach the canister', async () => {
			...
			mockCMCCanister.notifyCreateCanister.mockResolvedValue(mockCanisterDetails.id);
