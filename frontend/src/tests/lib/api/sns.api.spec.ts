...
import { ICPToken, LedgerCanister, TokenAmount, type SnsWasmCanisterOptions } from '@dfinity/nns';
...
describe('sns-api', () => {
	...
	beforeEach(() => {
		jest.spyOn(LedgerCanister, 'create').mockImplementation(() => ledgerCanisterMock);
	...
	it('should not participate in a swap if notifying nnsdapp fails', async () => {
		const nnsDappMock = mock<NNSDappCanister>();
		nnsDappMock.addPendingNotifySwap.mockRejectedValue(new NotAuthorizedError());
		jest.spyOn(NNSDappCanister, 'create').mockImplementation(() => nnsDappMock);
