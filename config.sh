...
json=$(HOST=$(api_host) jq -s --sort-keys '
  ...
  . as $config |
  .HOST=env.HOST |
  .GOVERNANCE_CANISTER_URL=( if (.GOVERNANCE_CANISTER_URL == null) then (env.HOST | sub("^(?<p>https?://)";"\(.p)\($config.GOVERNANCE_CANISTER_ID).")) else .GOVERNANCE_CANISTER_URL end ) |
  .LEDGER_CANISTER_URL=( if (.LEDGER_CANISTER_URL == null) then (env.HOST | sub("^(?<p>https?://)";"\(.p)\($config.LEDGER_CANISTER_ID).")) else .LEDGER_CANISTER_URL end ) |
  .OWN_CANISTER_URL=( if (.OWN_CANISTER_URL == null) then (env.HOST | sub("^(?<p>https?://)";"\(.p)\($config.OWN_CANISTER_ID).")) else .OWN_CANISTER_URL end ) |
