...
describe("vote", () => {
  ...
  it("getIcp", async () => {
    ...
    if (balance < 20){
	    ...
	    await browser.waitUntil(async () => submitButton.isClickable(), {timeoutMsg: "Timeout waiting for submitButton to be clickable."});
  ...
  it("goToNeuronTab", async() => {
    ...
    await browser.waitUntil(async () => tabButton.isClickable(), {timeoutMsg: "Timeout waiting for the neuron tab button."});
  ...
  it("createNeuronIfNone", async () => {
	...
	await browser.waitUntil(async () => updateDissolveDelayButton.isClickable(), {timeoutMsg: "Timeout waiting for updateDissolveDelayButton to be clickable."});
	updateDissolveDelayButton.click();
	const confirmUpdateButton = await browser.$('#modalContent [data-tid="confirm-delay-button"]');
	await browser.waitUntil(async () => confirmUpdateButton.isClickable(), {timeoutMsg: "Timeout waiting for confirmUpdateButton to be clickable."});
	...
	await browser.waitUntil(async () => closeFollowingModal.isClickable(), {timeoutMsg: "Timeout waiting for the closeFollowingModal button to be clickable."});
  ...
  it('hide-unavailable-proposals', async() => {
    ...
    const hideUnavailableCheckbox = await browser.$('label[for="hide-unavailable-proposals"]');
  ...
  it('votes', async() => {
    ...
    await browser.waitUntil(async () => yesMan.isClickable(), {timeoutMsg: "Timeout waiting for the yes vote button to be clickable."});
    ...
    await browser.waitUntil(async () => confirmButton.isClickable(), {timeoutMsg: "Timeout waiting for the confirm-yes button to be clickable."});
