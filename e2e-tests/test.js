...
try {
    ...
    console.log(`Could not read 'nns-dapp' local canister ID from ${CANISTER_IDS_PATH}`);
    throw e;
}
...
const proxy = child_process.spawn("proxy", ["--replica-host", replica_host, `${canister_id}:${NNS_DAPP_PORT}`]);
...
const wdio = child_process.spawn("npm", ["run", "wdio"], { env: { ...process.env, NNS_DAPP_URL: NNS_DAPP_URL}  });
