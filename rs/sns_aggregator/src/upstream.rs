...
pub async fn update_cache() {
    ...
    let sns_maybe = STATE.with(|state| {
        state.stable.sns_cache.borrow_mut().last_partial_update = time();
        state.stable.sns_cache.borrow_mut().sns_to_get.pop()
    });
    ...
    let result = if let Some((index, sns)) = sns_maybe {
        ...
        STATE.with(|state| {
            state.stable.sns_cache.borrow_mut().last_update = time();
        });
...
async fn set_list_of_sns_to_get() -> anyhow::Result<()> {
    ...
    match result {
        ...
        Ok((stuff,)) => {
            ...
            STATE.with(|state| {
                state.stable.sns_cache.borrow_mut().sns_to_get = instances;
            });
            Ok(())
        }
    }
