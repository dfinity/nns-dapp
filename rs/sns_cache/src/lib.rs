...
fn greet(name: String) -> String {
    STATE.with(|state| {
        let last_partial_update = state.sns_cache.borrow().last_partial_update;
        let last_update = state.sns_cache.borrow().last_update;
        let num_to_get = state.sns_cache.borrow().sns_to_get.len();
