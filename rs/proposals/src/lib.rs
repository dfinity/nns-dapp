...
/// Rust types can include things such as functions.  IDL types, sent over a wire, cannot.  Given that we want an `IDLType` from a more general type we need to convert the general type to the more specialized type.
