...
#[derive(CandidType, Deserialize)]
pub struct AccountIdentifier { bytes: serde_bytes::ByteBuf }
...
#[derive(CandidType, Deserialize)]
pub struct SubnetTypesToSubnetsResponse { data: Vec<(String,Vec<Principal>,)> }
...
#[derive(CandidType, Deserialize)]
pub struct NotifyTopUpArg { block_index: BlockIndex, canister_id: Principal }
...
impl Service {
  pub async fn get_icp_xdr_conversion_rate(&self) -> Result<
  ...
  pub async fn get_subnet_types_to_subnets(&self) -> Result<
  ...
  pub async fn notify_top_up(&self, arg0: NotifyTopUpArg) -> Result<
    (NotifyTopUpResult,)
