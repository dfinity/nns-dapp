...
impl fmt::Debug for AccountsStore {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(
            f,
            "AccountsStore{{StableBTreeMap{{.. {} entries}}, accounts_db_stats: {:?}}}",
...
impl AccountsStore {
    ...
    pub(crate) fn range(
        &self,
        key_range: impl std::ops::RangeBounds<Vec<u8>>,
    ) -> Box<dyn Iterator<Item = (Vec<u8>, Account)> + '_> {
        let iterator = self.accounts_db.range(key_range);
        Box::new(iterator)
    }
}
