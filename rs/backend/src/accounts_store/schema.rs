...
impl s0::AccountsDbS0Trait for AccountsStore {
  fn s0_get_account_page(&self, account_storage_key: &s0::AccountStorageKey) -> Option<s0::AccountStoragePage> {
    crate::state::ACCOUNTS_MEMORY_A.with(|accounts_memory_a| {
      accounts_memory_a.borrow().get(account_storage_key)
    })
  }
  ...
  fn s0_insert_account_page(
    ...
    crate::state::ACCOUNTS_MEMORY_A.with(|accounts_memory_a| {
      accounts_memory_a.borrow_mut().insert(account_storage_key, account)
    })
  }

  fn s0_contains_account_page(&self, account_storage_key: &s0::AccountStorageKey) -> bool {
    crate::state::ACCOUNTS_MEMORY_A.with(|accounts_memory_a| {
      accounts_memory_a.borrow().contains_key(account_storage_key)
    })
  }

  fn s0_remove_account_page(&mut self, account_storage_key: &s0::AccountStorageKey) -> Option<s0::AccountStoragePage> {
    crate::state::ACCOUNTS_MEMORY_A.with(|accounts_memory_a| {
      accounts_memory_a.borrow_mut().remove(account_storage_key)
    })
  }
}